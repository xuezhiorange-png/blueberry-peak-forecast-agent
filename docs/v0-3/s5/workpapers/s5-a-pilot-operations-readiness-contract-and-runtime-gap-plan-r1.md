...
The counters are deliberately separated. The retained
`S5_A_EXTERNAL_DECISION_GAP_COUNT=2` is only the dedicated `S5A-EXT-*` gap
count; it is not the count of all gaps whose matrix row requires an external
decision.
...
The five external decision records use independent implementation and
acceptance gates:

| Decision ID | REQUIRED_BEFORE_IMPLEMENTATION | REQUIRED_BEFORE_ACCEPTANCE |
| --- | --- | --- |
| S5A-DEC-001 | true | true |
| S5A-DEC-002 | true | true |
| S5A-DEC-003 | true | true |
| S5A-DEC-004 | false | true |
| S5A-DEC-005 | true | true |
