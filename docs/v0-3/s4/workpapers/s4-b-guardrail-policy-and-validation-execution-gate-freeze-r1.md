...
candidate identity and ordinal, counts, dataset identities, metric contract,
TEST seal, frozen execution manifest, parameter manifest, code commit, seed,
and evaluation identity. Unknown candidates, hash mismatches, exhausted
budgets, TEST access, missing manifests, native-float policy payloads, and
retry identity reuse are rejected. A retry must use a new evaluation ID and
reference its prior evaluation ID.
