...
Metric formulas are not changed by this plan. The current authority
reconciliation is:
...
The S3-B R2 verified-claim evidence is the current authority for quantile
semantics; historical failed R1 artifacts remain preserved and are not
rewritten. The S1 canonical current-main reconciliation is the authority for
metric-contract acceptance and threshold-policy acceptance. The older S3 rowset
snapshot is historical context, not a current S4 blocker; no post-S3-closeout
authority establishes it as an S4-A prerequisite. Candidate-specific guardrail
thresholds remain deliberately unresolved and are not invented. Metric
definition acceptance is distinct from metric execution: S4 metric execution
and result issuance remain false, and no candidate has run.
