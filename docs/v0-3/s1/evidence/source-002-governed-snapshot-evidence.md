...
COVERAGE_SCOPE_IDENTITY_PACKAGE_REFERENCE=source-002-coverage-scope-entity-identities-preparation
COVERAGE_SCOPE_IDENTITY_PACKAGE_REVIEW_STATUS=PASS
...
COVERAGE_SCOPE_KNOWN_BOUNDARIES=
  ...
  FORMAL_ENTITY_IDENTITY_ARRAYS_NOT_STORED_IN_GIT,
  GOVERNED_PACKAGE_REFERENCE_ONLY
COVERAGE_SCOPE_ENTITY_ID_LISTS=GOVERNED_PACKAGE_REFERENCE_ONLY_ARRAYS_NOT_STORED_IN_GIT
```

No farm, subfarm, or variety name list is included in Git. The reviewed
preparation package is bound by deterministic counts and array hashes only;
formal mapping remains unaccepted and a schema-valid source-cohort manifest is
still a separate artifact.
