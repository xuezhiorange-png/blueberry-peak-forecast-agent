...
runtime field set; there is no separate initial-status authority. Twelve
required rows remain blocked because other gate-specific evidence and final
independent review are not present. Source Authority is accepted by the
...
also `PASS` from the PR #243 exact-head reviewed Q2C closeout; no row-level data
...
CURRENT_REQUIRED_GATE_PASS_COUNT=5
CURRENT_REQUIRED_GATE_BLOCKED_COUNT=12
...
Authority, Source Cohort, and Q2C target gates as closed, but it cannot close
Physical Meaning, Unit/Time Basis, canonical grain, split, holdout-feasibility,
