...
R1 source lookup facts remain historical evidence only. They no longer serve as
the current conclusion that business data was not provided. R2 records that
the current runtime has historical receipt files and header-only templates,
but no row-bearing current location/variety/area scope.
