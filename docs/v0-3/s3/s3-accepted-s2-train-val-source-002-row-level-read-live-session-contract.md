# V0.3-S3-A2 Accepted S2 TRAIN/VALIDATION SOURCE_002 Row-Level Read Live Session Contract

## Contract identity and phase boundary

~~~text
CONTRACT_ID=V0_3_S3_A2_ACCEPTED_S2_TRAIN_VAL_SOURCE_002_ROW_LEVEL_READ_LIVE_SESSION_CONTRACT
CONTRACT_VERSION=v0-3-s3-a2-accepted-s2-train-val-source-002-row-level-read-live-session-contract-v1
TASK_ID=V03_S3_A2_ACCEPTED_S2_TRAIN_VAL_SOURCE_002_ROW_LEVEL_READ_LIVE_SESSION_CONTRACT_R1
TASK_CLASS=CONTRACT_DEFINITION_ONLY
AUTHORIZATION_SCOPE=S3_A2_ACCEPTED_S2_TRAIN_VAL_SOURCE_002_ROW_LEVEL_READ_LIVE_SESSION_CONTRACT_ONLY
SLICE=V0.3-S3
...
BASE_MAIN_SHA=e9f0fbb87c660e154fffd47f85b5122b9a281d2b
BASE_MAIN_TREE_SHA=2bacaa62b60c263dc851f7b179985ebdc6bc9f9d
...
live-session wiring** authority for a dedicated **live session** slice. The user
authorized a **new family** with `USER_GATE=授权`. The parent SOURCE_002
row-level-read family (#410–#413) already froze the deterministic reader
attestation slice, authorized it in live §4.4, issued the implementation grant,
and landed the deterministic reader on main. That parent family is **not
closed**. Official TRAIN+VAL content hashes have not been attested from a live
read. Unique live flip of `SOURCE_002_ROW_LEVEL_READ` remains reserved for that
parent family.

This contract authorizes a **future** deterministic service to bind a live
session provider into the already-landed reader — without executing that
wiring today, without inventing a DSN, without inserting live §4.4, issuing a
grant, or running R1, and without flipping live `SOURCE_002_ROW_LEVEL_READ` or
parent `IMPLEMENTED`. Binding a session that then fail-closes (no accepted
dataset, missing TRAIN/VAL bytes, hash mismatch, count mismatch, or TEST
unsealed) is not `SOURCE_002_ROW_LEVEL_READ`.

Merging this contract ≠ implementation authorized ≠ implemented ≠ live session
bound ≠ `SOURCE_002_ROW_LEVEL_READ` ≠ kg actually read ≠ identity-set member
...
`e9f0fbb`, `DETERMINISTIC_ACCEPTED_S2_TRAIN_VAL_LAWFUL_ORIGIN_IMPLEMENTED=true` in
live §4.4. Origin naming ≠ kg row-level read ≠ `SOURCE_002_ROW_LEVEL_READ` ≠
live session bound. This family does not reopen origin freeze, live-authority,
...
actually read ≠ `SOURCE_002_ROW_LEVEL_READ` ≠ live session bound. This family does
not supersede kg-read and must not copy that docs-only `IMPLEMENTED` pattern as
a substitute for binding a session or for the parent-family read.

### 1.5 Parent SOURCE_002 row-level-read family (#410–#413, not closed)

At base `e9f0fbb` live §4.4 holds:
...
Parent R1 (#413) landed
`backend/app/s3_daily_rowset/accepted_s2_train_val_source_002_row_level_read.py`
(blob `fc08f53cc493949bccf9d680cd85ad4beb189930`). That reader hashes persisted
accepted TRAIN/VALIDATION `content_bytes` against copied S2 official hashes and
fail-closes without a session or when official bytes are absent. Default session
provider is unset (`FAIL_CLOSED_NO_SESSION`). Official hashes were not attested
...
### 1.6 Populated-origin closed family
...
### 1.7 C0 §5 and P0 §11 (historical snapshots only)
...
2. Default session provider on that reader is unset; attestation without an
   injected session is fail-closed.
3. Parent unique remaining gap (official TRAIN+VAL hash attestation from a live
   read) is **not** this family's unique remaining gap and is **not** closed by
   this freeze.
4. The user authorized a **new family** to freeze WHAT/HOW for binding a live
   session into that landed reader. This slice must not uniquely flip
   `SOURCE_002_ROW_LEVEL_READ`.
5. The gap is **contract boundary for live-session wiring**, not executing that
   wiring today, not attesting official hashes, not re-naming origin, not
   reopening kg-read or parent freeze, not member enumeration, and not
   producing forecast artifact files.

## 3. Frozen live-session target (not the kg read itself)
...
Live-session wiring rules:

~~~text
THIS_FAMILY_IS_THE_LIVE_SESSION_WIRING_SLICE_FOR_THE_LANDED_SOURCE_002_ROW_LEVEL_READER=true
THIS_FAMILY_IS_NOT_THE_DETERMINISTIC_READER_ATTESTATION_SLICE=true
THIS_FAMILY_AUTHORIZES_FUTURE_BINDING_OF_A_LIVE_SESSION_INTO_THE_LANDED_READER=true
THIS_PR_DOES_NOT_BIND_A_LIVE_SESSION=true
THIS_PR_DOES_NOT_ATTEST_OFFICIAL_HASHES_FROM_A_LIVE_READ=true
THIS_FAMILY_MUST_NOT_UNIQUELY_FLIP_SOURCE_002_ROW_LEVEL_READ=true
PARENT_FAMILY_HOLDS_UNIQUE_LIVE_FLIP_OF_SOURCE_002_ROW_LEVEL_READ=true
...
BOUND_LIVE_SESSION_IS_NOT_SOURCE_002_ROW_LEVEL_READ=true
FAIL_CLOSED_AFTER_SESSION_INJECTION_IS_NOT_SOURCE_002_ROW_LEVEL_READ=true
LIVE_SESSION_WIRING_IS_NOT_IDENTITY_SET_MEMBER_ENUMERATION=true
LIVE_SESSION_WIRING_IS_NOT_FORECAST_ARTIFACT=true
LIVE_SESSION_WIRING_DOES_NOT_REWRITE_POPULATED_ORIGIN_FAIL_CLOSED=true
...
Future implementation of **this** family may bind only an injected session
provider into the landed reader. It must not invent a DSN, must not
rematerialize from xls / Sheets / S1 JSON / PIT / old-winner / raw primary, must
not unseal TEST, and must not treat H7 fixture
...
for actually binding a session. Binding a session that then fail-closes is not
parent `IMPLEMENTED` and is not `SOURCE_002_ROW_LEVEL_READ`.

## 4. Relationship to parent, origin, and kg-read families

- Parent reader landed ≠ official hashes attested ≠ `SOURCE_002_ROW_LEVEL_READ`
- Origin `IMPLEMENTED=true` ≠ `SOURCE_002_ROW_LEVEL_READ` ≠ live session bound
- Kg-read `IMPLEMENTED=true` ≠ `SOURCE_002_ROW_LEVEL_READ` ≠ kg read performed
- This family does not supersede parent attestation, origin naming, or kg-read
  target freeze
...
- This family's later unique legal flip, if any, is
  ...
  after a session is actually bound — not `SOURCE_002_ROW_LEVEL_READ`
...
- Empty-table DISTINCT / empty obtain() / zero-row live read are not kg read
  evidence, not `SOURCE_002_ROW_LEVEL_READ` evidence, and not live-session
  evidence for this harvest reader
- H7 fixture hash `8e74d6be6bcadc087b2dd7a72dfcb588e849305db598aac5c02a954660f30c18`
  must not be treated as live evidence, content identity, or this wiring target

## 6. Frozen-time honest blockers (not flipped)

Live §4.4 facts at base `e9f0fbb`:
...
FORBIDDEN_DOCS_ONLY_IMPLEMENTED_AS_SUBSTITUTE_FOR_BINDING_A_SESSION=true
~~~
...
implementation. Future implementation R1 of **this** family may bind a live
session into the landed reader. That R1 still must not uniquely flip
`SOURCE_002_ROW_LEVEL_READ`. The unique live flip of `SOURCE_002_ROW_LEVEL_READ`
...
- 本 file fence `AUTHORIZED=true` ≠ live §4.4 ≠ `IMPLEMENTATION_AUTHORIZED` ≠
  `IMPLEMENTED` ≠ live session 已绑定 ≠ `SOURCE_002_ROW_LEVEL_READ`
...
- 本 freeze ≠ 改写 origin / kg-read / parent SOURCE_002 freeze 历史快照
- 绑定 live session ≠ `SOURCE_002_ROW_LEVEL_READ`；注入后 fail-closed 也不是
- 本家族是【给已落地 reader 绑定 live session】的独立 slice；本 freeze 仍不执行该
  接线
...
~~~text
WORKPAPER_PATH=docs/v0-3/s3/workpapers/s3-accepted-s2-train-val-source-002-row-level-read-live-session-contract.md
EVIDENCE_JSON_PATH=docs/v0-3/s3/evidence/s3-accepted-s2-train-val-source-002-row-level-read-live-session-contract.json
~~~

## 13. S3-A2 accepted S2 TRAIN/VAL SOURCE_002 row-level-read live-session contract live-authority pointer
...
## 14. S3-A2 accepted S2 TRAIN/VAL SOURCE_002 row-level-read live-session authorization pointer
...
## 15. S3-A2 accepted S2 TRAIN/VAL SOURCE_002 row-level-read live-session R1 pointer
