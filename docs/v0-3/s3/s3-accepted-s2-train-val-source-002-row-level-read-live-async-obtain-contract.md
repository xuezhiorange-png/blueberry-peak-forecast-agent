...
~~~text
CONTRACT_ID=V0_3_S3_A2_ACCEPTED_S2_TRAIN_VAL_SOURCE_002_ROW_LEVEL_READ_LIVE_ASYNC_OBTAIN_CONTRACT
...
live async obtain** authority for a dedicated **live async obtain** slice. The user
authorized a **new family** with `USER_GATE=授权`. The parent SOURCE_002
row-level-read family (#410–#413) remains **not closed**: official TRAIN+VAL
content hashes have not been attested from a live read. Unique live flip of
`SOURCE_002_ROW_LEVEL_READ` remains reserved for that parent family.

The live-session family (#414–#417) already bound the default live session
provider into the landed reader. That family's unique remaining gap is
**closed**. Binding a live session is not `SOURCE_002_ROW_LEVEL_READ`. At base
`30ee96a`, accepted S2 TRAIN/VALIDATION `content_bytes` have **not** been
obtained through the already-obtained live AsyncSession.

This contract authorizes a **future** deterministic service to obtain accepted
TRAIN/VALIDATION `content_bytes` through the already-obtained live AsyncSession —
without executing that obtain today, without inventing a DSN, without
inserting live §4.4, issuing a grant, or running R1, and without flipping live
`SOURCE_002_ROW_LEVEL_READ` or parent `IMPLEMENTED`. Obtaining `content_bytes`
that then fail to match official hashes is not `SOURCE_002_ROW_LEVEL_READ`.

Merging this contract ≠ implementation authorized ≠ implemented ≠ content bytes
obtained ≠ `SOURCE_002_ROW_LEVEL_READ` ≠ kg actually read ≠ identity-set member
...
`30ee96a`, `DETERMINISTIC_ACCEPTED_S2_TRAIN_VAL_LAWFUL_ORIGIN_IMPLEMENTED=true` in
live §4.4. Origin naming ≠ kg row-level read ≠ `SOURCE_002_ROW_LEVEL_READ` ≠
content bytes obtained. This family does not reopen origin freeze, live-authority,
grant, or R1 evidence.
...
actually read ≠ `SOURCE_002_ROW_LEVEL_READ` ≠ content bytes obtained. This family
does not supersede kg-read and must not copy that docs-only `IMPLEMENTED`
pattern as a substitute for obtaining content bytes or for the parent-family
read.

### 1.5 Parent SOURCE_002 row-level-read family (#410–#413, not closed)

At base `30ee96a` live §4.4 holds:
...
Parent R1 (#413) landed the deterministic reader. Live-session R1 (#417) later
bound the default live session provider (reader blob
`2a9232064179da89484d52dcf203c95a0fa71a68`). Official hashes were not attested
from a live read. Parent unique remaining gap remains
...
is closed. Already-obtained live AsyncSession ≠ official TRAIN/VAL `content_bytes` obtained ≠
`SOURCE_002_ROW_LEVEL_READ`. `#414` freeze identity
...
### 1.7 Populated-origin closed family
...
### 1.8 C0 §5 and P0 §11 (historical snapshots only)
...
2. Live-session R1 already bound the default live session provider into that
   reader. That family's unique remaining gap is closed.
3. Live-async-session R1 (#437) already obtained an AsyncSession from the already-configured live AsyncSessionMaker. That family's unique remaining gap is closed.
4. Accepted S2 TRAIN/VALIDATION `content_bytes` have not been obtained through that already-obtained live AsyncSession.
5. Parent unique remaining gap (official TRAIN+VAL hash attestation from a live
   read) is **not** this family's unique remaining gap and is **not** closed by
   this freeze.
6. The user authorized a **new family** to freeze WHAT/HOW for obtaining
   TRAIN/VAL `content_bytes` through the already-obtained live AsyncSession. This slice must not
   uniquely flip `SOURCE_002_ROW_LEVEL_READ`.
7. The gap is **contract boundary for live async obtain**, not executing that obtain
   today, not attesting official hashes, not rebinding a session, not re-naming
   origin, not reopening kg-read / parent SOURCE_002 / live-session freeze, not
   member enumeration, and not producing forecast artifact files.

## 3. Frozen live-async-obtain target (not the kg read itself)
...
THIS_FAMILY_IS_NOT_THE_DETERMINISTIC_READER_ATTESTATION_SLICE=true
THIS_FAMILY_IS_NOT_THE_LIVE_SESSION_WIRING_SLICE=true
THIS_FAMILY_AUTHORIZES_FUTURE_OBTAIN_OF_TRAIN_VAL_CONTENT_BYTES_THROUGH_THE_ALREADY_OBTAINED_LIVE_ASYNC_SESSION=true
THIS_PR_DOES_NOT_OBTAIN_CONTENT_BYTES=true
THIS_PR_DOES_NOT_ATTEST_OFFICIAL_HASHES_FROM_A_LIVE_READ=true
THIS_PR_DOES_NOT_BIND_A_LIVE_SESSION=true
THIS_FAMILY_MUST_NOT_UNIQUELY_FLIP_SOURCE_002_ROW_LEVEL_READ=true
...
OBTAINED_CONTENT_BYTES_ARE_NOT_SOURCE_002_ROW_LEVEL_READ=true
FAIL_CLOSED_AFTER_SESSION_INJECTION_IS_NOT_SOURCE_002_ROW_LEVEL_READ=true
LIVE_ASYNC_OBTAIN_IS_NOT_IDENTITY_SET_MEMBER_ENUMERATION=true
LIVE_ASYNC_OBTAIN_IS_NOT_FORECAST_ARTIFACT=true
LIVE_ASYNC_OBTAIN_DOES_NOT_REWRITE_POPULATED_ORIGIN_FAIL_CLOSED=true
...
Future implementation of **this** family may obtain accepted TRAIN/VALIDATION
`content_bytes` through the already-obtained live AsyncSession. It must not invent a
DSN, must not rematerialize from xls / Sheets / S1 JSON / PIT / old-winner /
raw primary, must not unseal TEST, and must not treat H7 fixture
...
for actually obtaining TRAIN/VAL `content_bytes`. Obtaining `content_bytes`
that then fail to match official hashes is not parent `IMPLEMENTED` and is not
`SOURCE_002_ROW_LEVEL_READ`.

## 4. Relationship to parent, live-session, live-async-session, live-obtain, origin, and kg-read families

- Parent reader landed ≠ official hashes attested ≠ `SOURCE_002_ROW_LEVEL_READ`
- Live async session obtained ≠ content bytes obtained ≠ `SOURCE_002_ROW_LEVEL_READ`
- Origin `IMPLEMENTED=true` ≠ `SOURCE_002_ROW_LEVEL_READ` ≠ content bytes obtained
- Kg-read `IMPLEMENTED=true` ≠ `SOURCE_002_ROW_LEVEL_READ` ≠ kg read performed
- This family does not supersede parent attestation, live-session wiring, origin
  naming, or kg-read target freeze
...
- This family's later unique legal flip, if any, is
  ...
  after TRAIN/VAL `content_bytes` are actually obtained through the already-obtained live AsyncSession — not `SOURCE_002_ROW_LEVEL_READ`
...
- Empty-table DISTINCT / empty replay-table read / zero-row live read are not
  kg read evidence, not `SOURCE_002_ROW_LEVEL_READ` evidence, and not live-async-obtain
  evidence for this harvest reader
- H7 fixture hash `8e74d6be6bcadc087b2dd7a72dfcb588e849305db598aac5c02a954660f30c18`
  must not be treated as live evidence, content identity, or this obtain target

## 6. Frozen-time honest blockers (not flipped)

Live §4.4 facts at base `30ee96a`:
...
implementation. Future implementation R1 of **this** family may obtain TRAIN/VAL
`content_bytes` through the already-obtained live AsyncSession. That R1 still must not uniquely
flip `SOURCE_002_ROW_LEVEL_READ`. The unique live flip of `SOURCE_002_ROW_LEVEL_READ`
requires a later implementation of the **parent** family that actually attests
TRAIN+VAL official content hashes from a live read. Docs-only PRs of this
family must not flip `SOURCE_002_ROW_LEVEL_READ`, must not enumerate member
literals, must not unseal TEST, must not invent SQL/DSN/member/hash/tonnes, and
must not flip `NO_VERSIONED`.
...
- parent reader 已落地 ≠ 官方 hash 已从 live read attestation ≠
  `SOURCE_002_ROW_LEVEL_READ`
- live AsyncSession 已取得 ≠ TRAIN/VAL `content_bytes` 已取得 ≠
  `SOURCE_002_ROW_LEVEL_READ`
...
- 本 freeze ≠ 改写 origin / kg-read / parent SOURCE_002 / live-session freeze
  历史快照
- 取得 `content_bytes` ≠ `SOURCE_002_ROW_LEVEL_READ`；取得后 hash 不匹配也不是
- 本家族是【经已取得的 live AsyncSession 取得 TRAIN/VAL content_bytes】的独立 slice；
  本 freeze 仍不执行该取得
...
## 13. S3-A2 accepted S2 TRAIN/VAL SOURCE_002 row-level-read live-async-obtain contract live-authority pointer
...
## 14. S3-A2 accepted S2 TRAIN/VAL SOURCE_002 row-level-read live-async-obtain authorization pointer
