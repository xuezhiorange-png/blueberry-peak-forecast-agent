...
~~~text
CONTRACT_ID=V0_3_S3_A2_ACCEPTED_S2_TRAIN_VAL_SOURCE_002_ROW_LEVEL_READ_LIVE_ASYNC_CONNECTION_CONTRACT
...
LIVE_CONNECTION_LIVE_AUTH_WORKPAPER_GIT_BLOB_SHA=ac7245b1db8ac30bc48af93796d30843030529a7
LIVE_CONNECTION_LIVE_AUTH_EVIDENCE_GIT_BLOB_SHA=a2e8258da41a5e82a7e299af68ca4aff946b97a1
...
ASYNC_CONNECTION_IS_NOT_QUERYABLE_SESSION=true
ASYNC_CONNECTION_IS_NOT_CONTENT_BYTES_OBTAINED=true
ASYNC_CONNECTION_IS_NOT_SOURCE_002_ROW_LEVEL_READ=true
THIS_FAMILY_DOES_NOT_AUTHORIZE_A_DOCS_ONLY_IMPLEMENTED_FLIP_AS_SUBSTITUTE_FOR_AN_ASYNC_CONNECTION_FROM_THE_ALREADY_CONFIGURED_LIVE_ASYNC_ENGINE=true
...
live async connection** authority for a dedicated **already-configured live AsyncEngine async
connection** slice. The user authorized a **new family** with `USER_GATE=授权`.
...
is closed. Bound live session ≠ queryable bound session ≠ sync connection from
that session's bind ≠ official TRAIN/VAL `content_bytes` obtained ≠
...
Obtaining an async connection from the already-configured live AsyncEngine is not
obtaining a sync connection from the bound live session's bind.

### 1.10 Populated-origin closed family
...
### 1.11 C0 §5 and P0 §11 (historical snapshots only)
...
5. Live-connection R1 landed the bind-connection service. Official live
   ...
   family's gap.
6. An asynchronous connection has not been obtained from the already-configured
   live AsyncEngine.
7. User authorized a **new family** to freeze WHAT/HOW for that async connection.
   This freeze does not execute that work, does not insert live §4.4, does not
   grant, does not run R1.

Merging this contract ≠ implementation authorized ≠ implemented ≠ async connection
obtained from the already-configured live AsyncEngine ≠ sync connection from bind ≠
session queryable ≠ content bytes obtained ≠ `SOURCE_002_ROW_LEVEL_READ`.

File fence `AUTHORIZED=true` ≠ live §4.4.


## 3. Frozen live-async-connection target (not the kg read itself)
...
THIS_FAMILY_IS_NOT_THE_LIVE_OBTAIN_SLICE_FOR_TRAIN_VAL_CONTENT_BYTES=true
THIS_FAMILY_IS_NOT_THE_BOUND_LIVE_SESSION_QUERYABLE_SLICE=true
THIS_FAMILY_AUTHORIZES_FUTURE_ASYNC_CONNECTION_FROM_THE_ALREADY_CONFIGURED_LIVE_ASYNC_ENGINE=true
THIS_PR_DOES_NOT_OBTAIN_AN_ASYNC_CONNECTION_FROM_THE_ALREADY_CONFIGURED_LIVE_ASYNC_ENGINE=true
LANDED_ASYNC_ENGINE_MODULE=backend/app/db/session.py
LANDED_ASYNC_ENGINE_MODULE_GIT_BLOB_SHA=49845a077d252af2a7a246fa25616d7595535037
LIVE_CONNECTION_MODULE=backend/app/s3_daily_rowset/accepted_s2_train_val_source_002_row_level_read_live_connection.py
...
FAIL_CLOSED_SESSION_UNREADABLE_IS_NOT_SOURCE_002_ROW_LEVEL_READ=true
FAIL_CLOSED_SESSION_NOT_SYNCHRONOUSLY_QUERYABLE_IS_NOT_SOURCE_002_ROW_LEVEL_READ=true
LIVE_ASYNC_CONNECTION_IS_NOT_IDENTITY_SET_MEMBER_ENUMERATION=true
LIVE_ASYNC_CONNECTION_IS_NOT_FORECAST_ARTIFACT=true
LIVE_ASYNC_CONNECTION_DOES_NOT_REWRITE_POPULATED_ORIGIN_FAIL_CLOSED=true
...
FORBIDDEN_CALL_CREATE_ENGINE=true
FORBIDDEN_REREAD_XLS=true
FORBIDDEN_RAW_SOURCE_002_PRIMARY_READ=true
~~~

Future implementation of **this** family may obtain an asynchronous connection
from the already-configured live AsyncEngine. It must not invent a DSN, must not
call create_engine, must not claim the Session itself is queryable, must not
...
for actually obtaining that async connection from the already-configured live AsyncEngine. A connection from bind
...
- Live session bound ≠ queryable bound session ≠ connection from bind ≠
  content bytes obtained ≠ `SOURCE_002_ROW_LEVEL_READ`
- Live-obtain service landed ≠ content bytes obtained ≠ `SOURCE_002_ROW_LEVEL_READ`
- Live-session-query probe landed ≠ bound session queryable ≠
  `SOURCE_002_ROW_LEVEL_READ`
- Live-connection service landed ≠ sync connection from bind ≠
  `SOURCE_002_ROW_LEVEL_READ`
- Async connection from the already-configured live AsyncEngine ≠ sync connection
...
- This family's later unique legal flip, if any, is
  ...
  after an async connection is actually obtained from the already-configured live
...
- Empty-table DISTINCT / empty replay-table read / zero-row live read are not
  ...
  evidence, not live-session-query evidence, and not this family's async-connection evidence
...
FORBIDDEN_CLOSE_LIVE_SESSION_QUERY_UNIQUE_REMAINING_GAP_IN_THIS_FAMILY=true
...
FORBIDDEN_REWRITE_LIVE_SESSION_QUERY_FREEZE_IDENTITY=true
FORBIDDEN_REWRITE_LIVE_SESSION_QUERY_FREEZE_FENCE=true
FORBIDDEN_REWRITE_LIVE_CONNECTION_FREEZE=true
FORBIDDEN_REWRITE_LIVE_CONNECTION_FREEZE_IDENTITY=true
FORBIDDEN_REWRITE_LIVE_CONNECTION_FREEZE_FENCE=true
FORBIDDEN_FLIP_LIVE_CONNECTION_IMPLEMENTED=true
FORBIDDEN_CLOSE_LIVE_CONNECTION_UNIQUE_REMAINING_GAP_IN_THIS_FAMILY=true
...
FORBIDDEN_TREAT_CONNECTION_FROM_BIND_AS_QUERYABLE_SESSION=true
FORBIDDEN_TREAT_CONNECTION_FROM_BIND_AS_SOURCE_002_ROW_LEVEL_READ=true
...
FORBIDDEN_DOCS_ONLY_IMPLEMENTED_AS_SUBSTITUTE_FOR_A_SYNC_CONNECTION_FROM_BIND=true
FORBIDDEN_DOCS_ONLY_IMPLEMENTED_AS_SUBSTITUTE_FOR_AN_ASYNC_CONNECTION_FROM_THE_ALREADY_CONFIGURED_LIVE_ASYNC_ENGINE=true
~~~
...
| Obtain async connection from already-configured live AsyncEngine | forbidden |
| Obtain sync connection from bound live session bind | forbidden |
...
async connection from the already-configured live AsyncEngine. That R1
...
SQL/DSN/member/hash/tonnes, must not call create_engine, and must not flip
`NO_VERSIONED`.
...
- 本家族是【从已配置的 live AsyncEngine 取得异步 connection】的独立 slice；
  本 freeze 仍不执行该工作
- async connection ≠ sync connection from bind
...
- live-connection unique remaining gap 仍留给 live-connection 家族
...
CONTRACT_MERGE_DOES_NOT_OBTAIN_AN_ASYNC_CONNECTION_FROM_THE_ALREADY_CONFIGURED_LIVE_ASYNC_ENGINE=true
CONTRACT_MERGE_DOES_NOT_OBTAIN_A_SYNC_CONNECTION_FROM_THE_BOUND_LIVE_SESSION_BIND=true
...
CONTRACT_MERGE_DOES_NOT_FLIP_LIVE_CONNECTION_IMPLEMENTED=true
...
CONTRACT_MERGE_DOES_NOT_REWRITE_LIVE_CONNECTION_FREEZE=true
CONTRACT_FILE_FENCE_IS_NOT_LIVE_REGISTRY_AUTHORITY=true
DEVELOPMENT_PLAN_UNCHANGED=true
~~~
...
## 13. S3-A2 accepted S2 TRAIN/VAL SOURCE_002 row-level-read live-async-connection contract live-authority pointer
...
## 14. S3-A2 accepted S2 TRAIN/VAL SOURCE_002 row-level-read live-async-connection authorization pointer
...
## 15. S3-A2 accepted S2 TRAIN/VAL SOURCE_002 row-level-read live-async-connection R1 pointer
