# V0.3-S3-A2 Accepted S2 TRAIN/VALIDATION SOURCE_002 Row-Level Read Live Connection Contract

## Contract identity and phase boundary

~~~text
CONTRACT_ID=V0_3_S3_A2_ACCEPTED_S2_TRAIN_VAL_SOURCE_002_ROW_LEVEL_READ_LIVE_CONNECTION_CONTRACT
CONTRACT_VERSION=v0-3-s3-a2-accepted-s2-train-val-source-002-row-level-read-live-connection-contract-v1
TASK_ID=V03_S3_A2_ACCEPTED_S2_TRAIN_VAL_SOURCE_002_ROW_LEVEL_READ_LIVE_CONNECTION_CONTRACT_R1
TASK_CLASS=CONTRACT_DEFINITION_ONLY
AUTHORIZATION_SCOPE=S3_A2_ACCEPTED_S2_TRAIN_VAL_SOURCE_002_ROW_LEVEL_READ_LIVE_CONNECTION_CONTRACT_ONLY
SLICE=V0.3-S3
...
BASE_MAIN_SHA=7a1047b2f9ea2d8ad9f6fc46e79cb2bf2f7768a4
BASE_MAIN_TREE_SHA=c48c66f1f3a3f259a28b5b005099718fa3841fb3
...
live connection** authority for a dedicated **bound live session bind
...
Merging this contract ≠ implementation authorized ≠ implemented ≠ connection
obtained from bind ≠ session queryable ≠ content bytes obtained ≠
...
### 1.9 Populated-origin closed family
...
### 1.10 C0 §5 and P0 §11 (historical snapshots only)
...
4. Live-session-query R1 already landed the query probe. Bound probe fail-closed
   `FAIL_CLOSED_SESSION_NOT_SYNCHRONOUSLY_QUERYABLE`. The bound live session is
   not synchronously queryable. That family's unique remaining gap is **not**
   this family's unique remaining gap and is **not** closed by this freeze.
5. A synchronous connection has not been obtained from the already-bound live
   session's bind.
6. The user authorized a **new family** to freeze WHAT/HOW for obtaining that
   synchronous connection from the already-bound live session's bind without
   inventing a DSN and without calling create_engine. This slice must not
   uniquely flip `SOURCE_002_ROW_LEVEL_READ`, must not flip live-obtain
   `IMPLEMENTED`, and must not flip live-session-query `IMPLEMENTED`.
7. The gap is **contract boundary for a sync connection from the bound live
   session's bind**, not executing that work today, not making the Session
   queryable, not obtaining content bytes, not attesting official hashes, not
   rebinding a session, not rewriting live-session / live-obtain /
   live-session-query freeze, not member enumeration, and not producing forecast
   artifact files.

## 3. Frozen live-connection target (not the kg read itself)
...
THIS_FAMILY_IS_NOT_THE_LIVE_OBTAIN_SLICE_FOR_TRAIN_VAL_CONTENT_BYTES=true
THIS_FAMILY_IS_NOT_THE_BOUND_LIVE_SESSION_QUERYABLE_SLICE=true
THIS_FAMILY_AUTHORIZES_FUTURE_SYNC_CONNECTION_FROM_BOUND_LIVE_SESSION_BIND=true
THIS_PR_DOES_NOT_OBTAIN_A_SYNC_CONNECTION_FROM_THE_BOUND_LIVE_SESSION_BIND=true
THIS_PR_DOES_NOT_MAKE_THE_BOUND_SESSION_QUERYABLE=true
THIS_PR_DOES_NOT_OBTAIN_CONTENT_BYTES=true
THIS_PR_DOES_NOT_ATTEST_OFFICIAL_HASHES_FROM_A_LIVE_READ=true
...
FAIL_CLOSED_SESSION_UNREADABLE_IS_NOT_SOURCE_002_ROW_LEVEL_READ=true
FAIL_CLOSED_SESSION_NOT_SYNCHRONOUSLY_QUERYABLE_IS_NOT_SOURCE_002_ROW_LEVEL_READ=true
LIVE_CONNECTION_IS_NOT_IDENTITY_SET_MEMBER_ENUMERATION=true
LIVE_CONNECTION_IS_NOT_FORECAST_ARTIFACT=true
LIVE_CONNECTION_DOES_NOT_REWRITE_POPULATED_ORIGIN_FAIL_CLOSED=true
...
Future implementation of **this** family may obtain a synchronous connection
from the already-bound live session's bind. It must not invent a DSN, must not
...
for actually obtaining that sync connection from bind. A connection from bind
...
## 4. Relationship to parent, live-session, live-obtain, live-session-query, origin, and kg-read families
...
- This family's later unique legal flip, if any, is
  ...
  after a sync connection is actually obtained from the bound live session's
  bind — not `SOURCE_002_ROW_LEVEL_READ`, not live-obtain `IMPLEMENTED`, and not
...
implementation. Future implementation R1 of **this** family may obtain a
synchronous connection from the already-bound live session's bind. That R1
...
- 本家族是【从已绑定 live session 的 bind 取得同步 connection】的独立 slice；
  本 freeze 仍不执行该工作
...
~~~text
WORKPAPER_PATH=docs/v0-3/s3/workpapers/s3-accepted-s2-train-val-source-002-row-level-read-live-connection-contract.md
EVIDENCE_JSON_PATH=docs/v0-3/s3/evidence/s3-accepted-s2-train-val-source-002-row-level-read-live-connection-contract.json
~~~

## 13. S3-A2 accepted S2 TRAIN/VAL SOURCE_002 row-level-read live-connection contract live-authority pointer
...
## 14. S3-A2 accepted S2 TRAIN/VAL SOURCE_002 row-level-read live-connection authorization pointer
...
## 15. S3-A2 accepted S2 TRAIN/VAL SOURCE_002 row-level-read live-connection R1 pointer
