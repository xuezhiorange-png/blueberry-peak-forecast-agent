...
~~~text
ARTIFACT_ID=V0_3_S3_A2_ACCEPTED_S2_TRAIN_VAL_SOURCE_002_ROW_LEVEL_READ_LIVE_CONNECTION_CONTRACT
ARTIFACT_VERSION=v0-3-s3-a2-accepted-s2-train-val-source-002-row-level-read-live-connection-contract-v1
...
CONTRACT_PATH=docs/v0-3/s3/s3-accepted-s2-train-val-source-002-row-level-read-live-connection-contract.md
...
row-level-read live-connection** contract freeze after user authorization
...
This PR freezes WHAT/HOW for the dedicated **bound live session bind connection**
slice: a **future** deterministic service may obtain a synchronous connection
from the already-bound live session's bind without inventing a DSN and without
...
connection obtained from bind ≠ session queryable ≠ content bytes obtained ≠
...
| # | path |
|---|------|
| 1 | `docs/v0-3/s3/s3-accepted-s2-train-val-source-002-row-level-read-live-connection-contract.md` |
| 2 | `docs/v0-3/s3/workpapers/s3-accepted-s2-train-val-source-002-row-level-read-live-connection-contract.md` |
| 3 | `docs/v0-3/s3/evidence/s3-accepted-s2-train-val-source-002-row-level-read-live-connection-contract.json` |
...
contract, live-session-query contract, or any A2 identity-set contract.
...
## 3. Frozen live-connection target
...
`FAIL_CLOSED_SESSION_NOT_SYNCHRONOUSLY_QUERYABLE`. This family freezes a sync
connection from the bound live session's bind only:
...
connection-from-bind evidence. H7 fixture hash
...
Only `S3_A2_ACCEPTED_S2_TRAIN_VAL_SOURCE_002_ROW_LEVEL_READ_LIVE_CONNECTION_CONTRACT_AUTHORIZED`
...
- Rewrite origin / kg-read / parent SOURCE_002 / live-session / live-obtain /
  live-session-query / populated-origin freeze
...
- Docs-only `IMPLEMENTED` as a substitute for a sync connection from bind,
...
~~~text
EVIDENCE_JSON_SHA256=720cc266f2215cd25a4d5fa380f5e4770e988e669a7460eb6e473ad6247b98e7
~~~
