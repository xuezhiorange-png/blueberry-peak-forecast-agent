...
~~~text
ARTIFACT_ID=V0_3_S3_A2_ACCEPTED_S2_TRAIN_VAL_SOURCE_002_ROW_LEVEL_READ_LIVE_ASYNC_SESSION_BIND_QUERY_CONTRACT
ARTIFACT_VERSION=v0-3-s3-a2-accepted-s2-train-val-source-002-row-level-read-live-async-session-bind-query-contract-v1
...
CONTRACT_PATH=docs/v0-3/s3/s3-accepted-s2-train-val-source-002-row-level-read-live-async-session-bind-query-contract.md
...
row-level-read live-async-session-bind-query** contract freeze after user
...
bind connection queryable** slice: a **future** deterministic service may probe whether
the AsyncConnection from `get_bind()` / `bind.connect()` is asynchronously queryable.
...
bind connection queryable ≠ content bytes obtained ≠ `SOURCE_002_ROW_LEVEL_READ`.
...
| # | path |
|---|------|
| 1 | `docs/v0-3/s3/s3-accepted-s2-train-val-source-002-row-level-read-live-async-session-bind-query-contract.md` |
| 2 | `docs/v0-3/s3/workpapers/s3-accepted-s2-train-val-source-002-row-level-read-live-async-session-bind-query-contract.md` |
| 3 | `docs/v0-3/s3/evidence/s3-accepted-s2-train-val-source-002-row-level-read-live-async-session-bind-query-contract.json` |
...
## 3. Frozen live-async-session-bind-query target
...
`get_bind()` / `bind.connect()`. It must not use engine or connection escape hatches,
and must not invent connection parameters or construct a new session maker.

## 4. Unique flip (file fence only)

Only `S3_A2_ACCEPTED_S2_TRAIN_VAL_SOURCE_002_ROW_LEVEL_READ_LIVE_ASYNC_SESSION_BIND_QUERY_CONTRACT_AUTHORIZED`
