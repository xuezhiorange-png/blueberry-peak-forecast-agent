...
~~~text
ARTIFACT_ID=V0_3_S3_A2_ACCEPTED_S2_TRAIN_VAL_SOURCE_002_ROW_LEVEL_READ_LIVE_SESSION_CONTRACT
ARTIFACT_VERSION=v0-3-s3-a2-accepted-s2-train-val-source-002-row-level-read-live-session-contract-v1
...
CONTRACT_PATH=docs/v0-3/s3/s3-accepted-s2-train-val-source-002-row-level-read-live-session-contract.md
...
row-level-read live-session** contract freeze after user authorization
...
This PR freezes WHAT/HOW for the dedicated **live-session wiring** slice: a
**future** deterministic service may bind a live session provider into the
already-landed reader. This is **not** `可以实施`, not a grant, not R1, not live
§4.4 authority, not session binding execution, not official-hash attestation,
not a live flip of `SOURCE_002_ROW_LEVEL_READ`, not parent `IMPLEMENTED`, not
identity-set member landing, not versioned forecast artifact production, and not
origin / kg-read / parent SOURCE_002 / populated-origin freeze rewrite.
...
live session bound ≠ `SOURCE_002_ROW_LEVEL_READ` ≠ kg read performed ≠ members
landed ≠ `NO_REVIEWED` flipped ≠ versioned forecast artifact exists ≠
...
| # | path |
|---|------|
| 1 | `docs/v0-3/s3/s3-accepted-s2-train-val-source-002-row-level-read-live-session-contract.md` |
| 2 | `docs/v0-3/s3/workpapers/s3-accepted-s2-train-val-source-002-row-level-read-live-session-contract.md` |
| 3 | `docs/v0-3/s3/evidence/s3-accepted-s2-train-val-source-002-row-level-read-live-session-contract.json` |
...
parent SOURCE_002 row-level-read contract, or any A2 identity-set contract.
...
copy that docs-only `IMPLEMENTED` pattern as a substitute for binding a session.
...
snapshots where frozen. This family does not rewrite them.

### 2.6 Populated-origin closed family
...
evidence, not `SOURCE_002_ROW_LEVEL_READ` evidence, and not this family's
live-session evidence. H7 fixture hash
`8e74d6be6bcadc087b2dd7a72dfcb588e849305db598aac5c02a954660f30c18` must not be
treated as live evidence or this wiring target.

## 5. Unique flip (file fence only)

Only `S3_A2_ACCEPTED_S2_TRAIN_VAL_SOURCE_002_ROW_LEVEL_READ_LIVE_SESSION_CONTRACT_AUTHORIZED`
...
`NO_VERSIONED`, `NO_REVIEWED`, `COMPLETENESS_VERIFIED`, C0/S3-D/metric STATUS,
TEST seal, S3-B coverage, S4, live §4.4 insert, kg-read three live keys, origin
three live keys.

## 6. Forbidden (summary)

- Bind a live session; attest official hashes from a live read
...
- Rewrite origin / kg-read / parent SOURCE_002 / populated-origin freeze
- Authorize S3-B coverage or S4; touch Python / Alembic / tests
- Treat H7 fixture as live evidence
- Treat kg-read `IMPLEMENTED=true` or parent reader-landed as
  `SOURCE_002_ROW_LEVEL_READ`
- Docs-only `IMPLEMENTED` as a substitute for binding a session or for the
  parent-family read
...
~~~text
EVIDENCE_JSON_SHA256=196c6197cffb641fa7f078a5c12ea5eb99b9f27f56170a9ca2d94a51b795aa71
~~~
