# V0.3-S3-A2 Incumbent forecast V0.2 replay-identity grain identity-set candidate-source acquisition implementation authorization

## Artifact identity

~~~text
ARTIFACT_ID=V0_3_S3_A2_INCUMBENT_FORECAST_V0_2_REPLAY_IDENTITY_GRAIN_IDENTITY_SET_CANDIDATE_SOURCE_ACQUISITION_IMPLEMENTATION_AUTHORIZATION
ARTIFACT_VERSION=s3-a2-incumbent-forecast-v0-2-replay-identity-grain-identity-set-candidate-source-acquisition-authorization-v1
TASK_ID=V03_S3_A2_INCUMBENT_FORECAST_V0_2_REPLAY_IDENTITY_GRAIN_IDENTITY_SET_CANDIDATE_SOURCE_ACQUISITION_IMPLEMENTATION_AUTHORIZATION_R1
TASK_CLASS=DOCS_ONLY_AUTHORIZATION_ISSUANCE
AUTHORIZATION_SCOPE=S3_A2_INCUMBENT_FORECAST_V0_2_REPLAY_IDENTITY_GRAIN_IDENTITY_SET_CANDIDATE_SOURCE_ACQUISITION_IMPLEMENTATION_GRANT_ONLY
SLICE=V0.3-S3
...
BASE_MAIN_SHA=72245ba51cad1594b2595789fd55a7640e3e9444
BASE_MAIN_TREE_SHA=5a456c8014db725f79cf1844f1ee22d6fa5852bb
PARENT_PR=378
...
WORKPAPER_PATH=docs/v0-3/s3/workpapers/s3-a2-incumbent-forecast-v0-2-replay-identity-grain-identity-set-candidate-source-acquisition-authorization.md
EVIDENCE_JSON_PATH=docs/v0-3/s3/evidence/s3-a2-incumbent-forecast-v0-2-replay-identity-grain-identity-set-candidate-source-acquisition-authorization.json
GRANT_ONLY=true
...
candidate-source acquisition** implementation grant after the acquisition contract freeze on main (#378). This document
records what a **later** acquisition R1 may do when the user again says 「可以实施」. This PR does not perform
acquisition R1, flip `DETERMINISTIC_INCUMBENT_FORECAST_V0_2_REPLAY_IDENTITY_GRAIN_IDENTITY_SET_CANDIDATE_SOURCE_ACQUISITION_IMPLEMENTED`,
acquire a candidate, land identity-set members, INSERT rows, invent member literals, flip
`NO_REVIEWED_GRAIN_IDENTITY_SET_IN_REPOSITORY`, flip `NO_VERSIONED`, or authorize production or test code mutation.

This is **candidate-source acquisition** implementation authorization only. Parent acquisition contract §§1–9,
...
No lawful populated candidate source exists today. Candidate-source R1 evidence is **not** an acquisition package.
`CANDIDATE_SOURCE_IMPLEMENTED=true` ≠ lawful populated source exists ≠ acquisition performed. This grant authorizes a
**later** acquisition R1 per parent acquisition contract §3.4 — not acquiring a candidate today, not landing members,
...
Acquisition contract ≠ this grant ≠ acquisition R1 ≠ candidate-source WHERE contract ≠ candidate-source R1 ≠
...
No lawful populated candidate source exists; table still 0 rows; default provider remains empty. Default `obtain()`
without session still `()`. Catalog first blocker remains `NO_VERSIONED_INCUMBENT_FORECAST_ARTIFACT`. This grant does
not close S3. Jumping to acquisition R1 now is forbidden.
...
Acquisition contract is frozen on main (#378). No lawful populated candidate source exists today. Candidate-source R1
evidence is not an acquisition package. This grant does not invent, enumerate, or exemplify any member-shape value lists
...
A later acquisition R1 may, under a separate user 「可以实施」 gate, record a hashable candidate-source acquisition
evidence package per parent acquisition contract §3.4. Acquisition R1 is **docs-only**; no production or test code
mutation is authorized by this grant.

### 4.1 Allowed changes (future implementation only)

~~~text
ACQUISITION_R1_IS_DOCS_ONLY=true
S3_PRODUCTION_CODE_MUTATION_AUTHORIZED=false
S3_TEST_CODE_MUTATION_AUTHORIZED=false
FORBIDDEN_TOUCH_TEST_CATALOG_ARTIFACT_PY=true
...
3. Must not enumerate member literals; must not promote loader-test-only members; must not derive from source-002 or
   H7; must not treat candidate-source R1 evidence as an acquisition package.

Acquisition R1 ≠ candidate-source WHERE ≠ candidate-source R1 ≠ independent-review ≠ landing ≠ `NO_REVIEWED` flip ≠
INSERT ≠ versioned artifact ≠ catalog closeout.

The only implementation-status flip permitted by a future acquisition R1 (when no lawful populated origin) is:

~~~text
DETERMINISTIC_INCUMBENT_FORECAST_V0_2_REPLAY_IDENTITY_GRAIN_IDENTITY_SET_CANDIDATE_SOURCE_ACQUISITION_IMPLEMENTED=false → true
~~~

### 4.2 What future acquisition R1 must keep false / unchanged
...
`NO_VERSIONED_INCUMBENT_FORECAST_ARTIFACT`. `CANDIDATE_SOURCE_IMPLEMENTED=true` ≠ acquisition performed.
...
~~~text
S3_A2_INCUMBENT_FORECAST_V0_2_REPLAY_IDENTITY_GRAIN_IDENTITY_SET_CANDIDATE_SOURCE_ACQUISITION_IMPLEMENTATION_AUTHORIZED=false → true
~~~

Locations:

- `docs/v0-3/development-plan.md` §4.4 live state block
- `docs/v0-3/s3/s3-incumbent-forecast-v0-2-replay-identity-grain-identity-set-candidate-source-acquisition-contract.md` §10 pointer
- `docs/v0-3/s3/s3-incumbent-forecast-v0-2-replay-identity-grain-identity-set-candidate-source-contract.md` §13 pointer
- `docs/v0-3/s3/s3-incumbent-forecast-v0-2-replay-identity-grain-identity-set-independent-review-contract.md` §16 pointer
- `docs/v0-3/s3/s3-incumbent-forecast-v0-2-replay-identity-grain-identity-set-landing-contract.md` §19 pointer
- `docs/v0-3/s3/s3-incumbent-forecast-v0-2-replay-identity-grain-identity-set-contract.md` §22 pointer
- `docs/v0-3/s3/s3-incumbent-forecast-v0-2-replay-identity-grain-row-presence-contract.md` §25 pointer
- `docs/v0-3/s3/s3-incumbent-forecast-v0-2-live-postgres-read-contract.md` §28 pointer
- `docs/v0-3/s3/s3-incumbent-forecast-replay-identity-bindable-name-contract.md` §31 pointer
- `docs/v0-3/s3/s3-incumbent-forecast-replay-identity-persistence-schema-contract.md` §34 pointer
- `docs/v0-3/s3/s3-incumbent-forecast-v0-2-sql-table-authority-contract.md` §37 pointer
- `docs/v0-3/s3/s3-s2-identity-alignment-harvest-source-contract.md` §40 pointer
- `docs/v0-3/s3/s3-s2-identity-alignment-producer-adapter-wiring-contract.md` §43 pointer
- `docs/v0-3/s3/s3-incumbent-forecast-v0-2-postgres-obtain-contract.md` §46 pointer
- `docs/v0-3/s3/s3-incumbent-forecast-fail-closed-wiring-contract.md` §49 pointer
- `docs/v0-3/s3/s3-incumbent-forecast-live-envelope-contract.md` §52 pointer
- `docs/v0-3/s3/s3-incumbent-forecast-live-source-kind-contract.md` §55 pointer
- `docs/v0-3/s3/s3-incumbent-forecast-replay-source-contract.md` §58 pointer
- `docs/v0-3/s3/s3-incumbent-forecast-artifact-content-contract.md` §61 pointer
- `docs/v0-3/s3/s3-accepted-s2-identity-alignment-evidence-contract.md` §63 pointer
- `docs/v0-3/s3/s3-s2-identity-alignment-contract.md` §69 pointer
- `docs/v0-3/s3/s3-incumbent-forecast-artifact-contract.md` §70 pointer
- `docs/v0-3/s3/s3-evaluation-instance-catalog-artifact-contract.md` §74 pointer
- `docs/v0-3/s3/s3-evaluation-instance-catalog-binding-contract.md` §75 pointer
- `docs/v0-3/s3/s3-evaluation-instance-registry-contract.md` §78 pointer
- `docs/v0-3/s3/s3-daily-rowset-amendment.md` §86 pointer
