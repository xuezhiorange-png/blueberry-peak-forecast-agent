...
~~~text
ARTIFACT_ID=V0_3_S3_A2_ACCEPTED_S2_TRAIN_VAL_KG_ROW_LEVEL_READ_CONTRACT
ARTIFACT_VERSION=v0-3-s3-a2-accepted-s2-train-val-kg-row-level-read-contract-v1
...
CONTRACT_PATH=docs/v0-3/s3/s3-accepted-s2-train-val-kg-row-level-read-contract.md
...
This workpaper records the S3-A2 **accepted S2 TRAIN/VALIDATION kg row-level read**
contract freeze after user authorization `USER_GATE=授权` for a **new family**.
Historical source-002 actuals are already accepted in S2 (#296); the origin family
(#402–#405) already named that dataset as lawful origin at the dataset-identity
layer. This PR freezes WHAT/HOW for a **future** deterministic service to read
kilogram rows from the accepted TRAIN and VALIDATION materialized partitions and,
after read, attest the official content hashes. This is **not** `可以实施`, not a
grant, not R1, not live §4.4 authority, not actual kg read execution, not
identity-set member landing, not versioned forecast artifact production, and not
...
`S3_A2_ACCEPTED_S2_TRAIN_VAL_KG_ROW_LEVEL_READ_CONTRACT_AUTHORIZED=true` (file fence) ≠
live §4.4 authority ≠ implementation authorized ≠ implemented ≠
`SOURCE_002_ROW_LEVEL_READ` ≠ kg read performed ≠ members landed ≠ `NO_REVIEWED` flipped
≠ versioned forecast artifact exists ≠ `NO_VERSIONED` flipped ≠ catalog bindable ≠
completeness `VERIFIED=true` ≠ backtest/attribution/metrics computed ≠ S3-B coverage ≠
S4 ≠ TEST unsealed. Catalog first blocker remains `NO_VERSIONED_INCUMBENT_FORECAST_ARTIFACT`.

## 1. Three-file manifest (exactly)

| # | path |
|---|------|
| 1 | `docs/v0-3/s3/s3-accepted-s2-train-val-kg-row-level-read-contract.md` |
| 2 | `docs/v0-3/s3/workpapers/s3-accepted-s2-train-val-kg-row-level-read-contract.md` |
| 3 | `docs/v0-3/s3/evidence/s3-accepted-s2-train-val-kg-row-level-read-contract.json` |

No fourth file. No Python, Alembic, tests, or edits to development-plan, amendment,
P0, C0, S3-D, metric, S3-B, populated-origin, origin contract, or any A2
...
### 2.4 Populated-origin closed family
...
### 2.5 Artifact contract (HOW, not kg read target)
...
ARTIFACT_CONTRACT_DEFINES_HOW_NOT_THIS_FAMILY_KG_READ=true
~~~

### 2.6 Completeness dataset-claim R1 (#401)
...
Official hashes copied from S2 acceptance / origin contract (not recomputed):
...
Read rules: kg row-level read of accepted TRAIN+VALIDATION partitions only; not member
enumeration; not forecast artifact; not populated-origin rewrite. Contract does not
enumerate farm/date/cutoff member literals or invent SQL/table names.
...
must not be treated as live evidence or this read target.

## 5. Unique flip (file fence only)

Only `S3_A2_ACCEPTED_S2_TRAIN_VAL_KG_ROW_LEVEL_READ_CONTRACT_AUTHORIZED` flips
...
- Execute kg row-level read; invent members, farm/date/cutoff lists, hashes, tonnes, SQL, or DSN
- `SOURCE_002_ROW_LEVEL_READ` or TEST unseal
- Live §4.4 insert; development-plan rewrite; C0 §5 rewrite; P0 §11 sixth row
- Rewrite origin contract; rewrite populated-origin freeze; append A2 identity-set pointers
- Authorize S3-B coverage or S4; touch Python / Alembic / tests
- Treat H7 fixture as live evidence or this read target
- Treat this evidence as versioned forecast artifact, completeness verified package, or backtest package
...
~~~text
EVIDENCE_JSON_SHA256=618b33a0ad903181f646fbd7b740e30fc136dbaa21e0b7d73334ce1141d0795c
~~~
