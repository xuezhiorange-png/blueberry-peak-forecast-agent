...
Synthetic unit OBTAINED path is not official live async session. This R1 does not
flip `DETERMINISTIC_ACCEPTED_S2_TRAIN_VAL_SOURCE_002_ROW_LEVEL_READ_LIVE_ASYNC_SESSION_IMPLEMENTED`,
does not flip live-async-connection `IMPLEMENTED`, does not flip live-connection
...
LIVE_ASYNC_SESSION_THROUGH_ALREADY_CONFIGURED_SESSION_MAKER_REASON_CODE=OBTAINED
SYNTHETIC_OBTAINED_UNIT_PATH_IS_NOT_OFFICIAL_LIVE_ASYNC_SESSION=true
DETERMINISTIC_ACCEPTED_S2_TRAIN_VAL_SOURCE_002_ROW_LEVEL_READ_LIVE_ASYNC_SESSION_IMPLEMENTED=false (unchanged)
