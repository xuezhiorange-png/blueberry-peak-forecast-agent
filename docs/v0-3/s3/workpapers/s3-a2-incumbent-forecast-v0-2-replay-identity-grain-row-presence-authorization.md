# V0.3-S3-A2 Incumbent forecast V0.2 replay-identity grain row presence implementation authorization

## Artifact identity

~~~text
ARTIFACT_ID=V0_3_S3_A2_INCUMBENT_FORECAST_V0_2_REPLAY_IDENTITY_GRAIN_ROW_PRESENCE_IMPLEMENTATION_AUTHORIZATION
ARTIFACT_VERSION=s3-a2-incumbent-forecast-v0-2-replay-identity-grain-row-presence-authorization-v1
TASK_ID=V03_S3_A2_INCUMBENT_FORECAST_V0_2_REPLAY_IDENTITY_GRAIN_ROW_PRESENCE_IMPLEMENTATION_AUTHORIZATION_R1
TASK_CLASS=DOCS_ONLY_AUTHORIZATION_ISSUANCE
AUTHORIZATION_SCOPE=S3_A2_INCUMBENT_FORECAST_V0_2_REPLAY_IDENTITY_GRAIN_ROW_PRESENCE_IMPLEMENTATION_GRANT_ONLY
SLICE=V0.3-S3
...
BASE_MAIN_SHA=8698d1d0796d5cc623c6c2f243afb740929f6825
BASE_MAIN_TREE_SHA=1cb301da5d6dfb8aac06b3c96dd5251a058dee10
PARENT_PR=363
...
WORKPAPER_PATH=docs/v0-3/s3/workpapers/s3-a2-incumbent-forecast-v0-2-replay-identity-grain-row-presence-authorization.md
EVIDENCE_JSON_PATH=docs/v0-3/s3/evidence/s3-a2-incumbent-forecast-v0-2-replay-identity-grain-row-presence-authorization.json
GRANT_ONLY=true
...
The user authorized issuance of the S3-A2 **incumbent forecast V0.2 replay-identity grain row presence**
implementation grant after the grain-row-presence contract freeze on main (#363). This document
records what a **later** deterministic grain-row-presence R1 may do when the user again says
「可以实施」. This PR does not implement grain row presence, flip
`GRAIN_ROW_PRESENCE_IMPLEMENTED`, populate rows, INSERT grain rows, invent identity-set values,
flip `NO_VERSIONED`, or authorize production code mutation.

This is **grain-row-presence** implementation authorization only. Parent grain-row-presence contract
§§1–9, parent live-postgres-read contract, and the frozen 106-row Alembic audit remain authoritative
...
Do not confuse with live-read R1 (already landed). Live-read wires session read of frozen table
`s3_incumbent_forecast_replay_identity`; this grant authorizes a **later** fail-closed INSERT-if-reviewed-set-else-0-rows
R1 per parent contract §3.3 — not live-read's second empty return replacement.
...
Grant ≠ grain-row-presence contract ≠ R1 ≠ INSERT ≠ identity-set invention ≠ versioned artifact ≠
catalog closeout. Empty table still 0 rows. Default `obtain()` without session still `()`. Session
read of empty table still `()`. Catalog first blocker remains `NO_VERSIONED_INCUMBENT_FORECAST_ARTIFACT`.
This grant does not close S3. Jumping to grain-row-presence R1 / INSERT now is forbidden.
...
UNIQUE_REMAINING_GAP=_empty_replay_identity_table_without_reviewed_grain_identity_set
OBJECT_ROW_COUNT_AT_REVIEW=0
NO_REVIEWED_GRAIN_IDENTITY_SET_IN_REPOSITORY=true
~~~

Live-read R1 is landed. Frozen table `s3_incumbent_forecast_replay_identity` still has 0 rows.
No coordinator-reviewed grain identity-set exists in repository. This grant does not invent,
enumerate, or exemplify any `forecast_cutoff_at` / `model_id` / `forecast_quantile`. This grant
does not change those Python bytes.
...
PARENT_GRAIN_ROW_PRESENCE_CONTRACT_GIT_BLOB_SHA_AT_FREEZE=636f2fa960dc8f8b5e58024ca7415a74d0f89a1d
GRAIN_ROW_PRESENCE_CONTRACT_EVIDENCE_JSON_SHA256=1e7204dc716dd1e65de4b0248ebe4de01638cb31628b0cb10db8e1fdf5aba833
PARENT_LIVE_POSTGRES_READ_CONTRACT_GIT_BLOB_SHA_AT_GRAIN_CONTRACT_FREEZE=2a5225f1a7b1fde3abb7c353fa5be3a9e545a61c
CURRENT_LIVE_POSTGRES_READ_CONTRACT_GIT_BLOB_SHA=ea867a9193240875c5b94defe03e588fa9afb72f
...
values. The parent 106-row Alembic audit is cited only; not rewritten by this authorization grant.
`s3_incumbent_forecast_replay_identity` exists in Alembic with 0 rows; `MATCH_TABLE_COUNT` remains `0`.
...
A later deterministic grain-row-presence R1 may, under a separate user 「可以实施」 gate,
INSERT grain rows into frozen table `s3_incumbent_forecast_replay_identity` per parent contract §3.3.

### 4.1 Allowed changes (future implementation only)

~~~text
OPTIONAL_LIMITED_PRODUCTION_CODE_GRAIN_ROW_PRESENCE=true
OPTIONAL_NEW_TESTS_GRAIN_ROW_PRESENCE=true
...
1. INSERT grain rows into frozen table `s3_incumbent_forecast_replay_identity` **only** when a
   separately coordinator-reviewed grain identity-set already exists in repository (not invented
   in grant or R1 docs).
2. Populate **only** grain columns `forecast_cutoff_at`, `model_id`, `forecast_quantile`. No
   kg/tonnes/weight/quantity/forecast_value/daily_curve/harvest_business_date/catalog_cell/
   alignment_identity.
3. If no independently reviewed identity-set exists at R1 time: fail-closed, remain 0 rows. Must
   not invent cutoff/model/quantile lists in R1 docs or code comments. Must not derive identity-set
   from source-002 row-level reads.
4. Flip `DETERMINISTIC_INCUMBENT_FORECAST_V0_2_REPLAY_IDENTITY_GRAIN_ROW_PRESENCE_IMPLEMENTED` to
   `true` in `docs/v0-3/development-plan.md` §4.4 live block. `IMPLEMENTED=true` ≠ rows exist ≠
   versioned artifact ≠ catalog closeout.
5. Add tests asserting: no reviewed identity-set → still 0 rows; session read of empty table → `()`;
   default obtain without session → `()`.
6. Optional limited production code serving fail-closed INSERT-if-reviewed-set-else-0-rows only.

The only implementation-status flip permitted by a future grain-row-presence R1 is:

~~~text
DETERMINISTIC_INCUMBENT_FORECAST_V0_2_REPLAY_IDENTITY_GRAIN_ROW_PRESENCE_IMPLEMENTED=false → true
~~~

### 4.2 What future grain-row-presence R1 must keep false / unchanged
...
Honesty: even after a future grain-row-presence R1 implements fail-closed wiring, without a
reviewed identity-set the table still has 0 rows and default `obtain()` remains `()`. Catalog first
...
FORBIDDEN_INVENT_IDENTITY_SET=true
...
TEST_INCUMBENT_FORECAST_V0_2_POSTGRES_OBTAIN_PY_BLOB=48ce541d6a62c2d31046b3c8f39ba4ba788d7f23
TEST_INCUMBENT_FORECAST_V0_2_LIVE_POSTGRES_READ_PY_BLOB=5695249691ddda49b7682455eb3ec8ca5b365890
...
~~~text
S3_A2_INCUMBENT_FORECAST_V0_2_REPLAY_IDENTITY_GRAIN_ROW_PRESENCE_IMPLEMENTATION_AUTHORIZED=false → true
~~~

Locations:

- `docs/v0-3/development-plan.md` §4.4 live state block
- `docs/v0-3/s3/s3-incumbent-forecast-v0-2-replay-identity-grain-row-presence-contract.md` §10 pointer
- `docs/v0-3/s3/s3-incumbent-forecast-v0-2-live-postgres-read-contract.md` §13 pointer
- `docs/v0-3/s3/s3-incumbent-forecast-replay-identity-bindable-name-contract.md` §16 pointer
- `docs/v0-3/s3/s3-incumbent-forecast-replay-identity-persistence-schema-contract.md` §19 pointer
- `docs/v0-3/s3/s3-incumbent-forecast-v0-2-sql-table-authority-contract.md` §22 pointer
- `docs/v0-3/s3/s3-s2-identity-alignment-harvest-source-contract.md` §25 pointer
- `docs/v0-3/s3/s3-s2-identity-alignment-producer-adapter-wiring-contract.md` §28 pointer
- `docs/v0-3/s3/s3-incumbent-forecast-v0-2-postgres-obtain-contract.md` §31 pointer
- `docs/v0-3/s3/s3-incumbent-forecast-fail-closed-wiring-contract.md` §34 pointer
- `docs/v0-3/s3/s3-incumbent-forecast-live-envelope-contract.md` §37 pointer
- `docs/v0-3/s3/s3-incumbent-forecast-live-source-kind-contract.md` §40 pointer
- `docs/v0-3/s3/s3-incumbent-forecast-replay-source-contract.md` §43 pointer
- `docs/v0-3/s3/s3-incumbent-forecast-artifact-content-contract.md` §46 pointer
- `docs/v0-3/s3/s3-accepted-s2-identity-alignment-evidence-contract.md` §48 pointer
- `docs/v0-3/s3/s3-s2-identity-alignment-contract.md` §54 pointer
- `docs/v0-3/s3/s3-incumbent-forecast-artifact-contract.md` §55 pointer
- `docs/v0-3/s3/s3-evaluation-instance-catalog-artifact-contract.md` §59 pointer
- `docs/v0-3/s3/s3-evaluation-instance-catalog-binding-contract.md` §60 pointer
- `docs/v0-3/s3/s3-evaluation-instance-registry-contract.md` §63 pointer
- `docs/v0-3/s3/s3-daily-rowset-amendment.md` §71 pointer
