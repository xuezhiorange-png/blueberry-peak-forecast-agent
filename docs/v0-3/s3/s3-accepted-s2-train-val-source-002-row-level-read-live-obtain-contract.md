# V0.3-S3-A2 Accepted S2 TRAIN/VALIDATION SOURCE_002 Row-Level Read Live Obtain Contract

## Contract identity and phase boundary

~~~text
CONTRACT_ID=V0_3_S3_A2_ACCEPTED_S2_TRAIN_VAL_SOURCE_002_ROW_LEVEL_READ_LIVE_OBTAIN_CONTRACT
CONTRACT_VERSION=v0-3-s3-a2-accepted-s2-train-val-source-002-row-level-read-live-obtain-contract-v1
TASK_ID=V03_S3_A2_ACCEPTED_S2_TRAIN_VAL_SOURCE_002_ROW_LEVEL_READ_LIVE_OBTAIN_CONTRACT_R1
TASK_CLASS=CONTRACT_DEFINITION_ONLY
AUTHORIZATION_SCOPE=S3_A2_ACCEPTED_S2_TRAIN_VAL_SOURCE_002_ROW_LEVEL_READ_LIVE_OBTAIN_CONTRACT_ONLY
SLICE=V0.3-S3
...
BASE_MAIN_SHA=915b625548e5fe3f509e695d115eb51d6f3c8675
BASE_MAIN_TREE_SHA=a7b6127bc7b8cf06801f293ae0c8886680dfebb4
...
live obtain** authority for a dedicated **live obtain** slice. The user
...
`915b625`, accepted S2 TRAIN/VALIDATION `content_bytes` have **not** been
obtained through that bound session.

This contract authorizes a **future** deterministic service to obtain accepted
TRAIN/VALIDATION `content_bytes` through the already-bound live session —
...
`915b625`, `DETERMINISTIC_ACCEPTED_S2_TRAIN_VAL_LAWFUL_ORIGIN_IMPLEMENTED=true` in
...
At base `915b625` live §4.4 holds:
...
is closed. Bound live session ≠ official TRAIN/VAL `content_bytes` obtained ≠
...
3. Accepted S2 TRAIN/VALIDATION `content_bytes` have not been obtained through
   that bound session.
4. Parent unique remaining gap (official TRAIN+VAL hash attestation from a live
   read) is **not** this family's unique remaining gap and is **not** closed by
   this freeze.
5. The user authorized a **new family** to freeze WHAT/HOW for obtaining
   TRAIN/VAL `content_bytes` through the bound live session. This slice must not
   uniquely flip `SOURCE_002_ROW_LEVEL_READ`.
6. The gap is **contract boundary for live obtain**, not executing that obtain
...
## 3. Frozen live-obtain target (not the kg read itself)
...
THIS_FAMILY_IS_NOT_THE_DETERMINISTIC_READER_ATTESTATION_SLICE=true
THIS_FAMILY_IS_NOT_THE_LIVE_SESSION_WIRING_SLICE=true
THIS_FAMILY_AUTHORIZES_FUTURE_OBTAIN_OF_TRAIN_VAL_CONTENT_BYTES_THROUGH_THE_BOUND_LIVE_SESSION=true
THIS_PR_DOES_NOT_OBTAIN_CONTENT_BYTES=true
THIS_PR_DOES_NOT_ATTEST_OFFICIAL_HASHES_FROM_A_LIVE_READ=true
THIS_PR_DOES_NOT_BIND_A_LIVE_SESSION=true
THIS_FAMILY_MUST_NOT_UNIQUELY_FLIP_SOURCE_002_ROW_LEVEL_READ=true
...
OBTAINED_CONTENT_BYTES_ARE_NOT_SOURCE_002_ROW_LEVEL_READ=true
FAIL_CLOSED_AFTER_SESSION_INJECTION_IS_NOT_SOURCE_002_ROW_LEVEL_READ=true
LIVE_OBTAIN_IS_NOT_IDENTITY_SET_MEMBER_ENUMERATION=true
LIVE_OBTAIN_IS_NOT_FORECAST_ARTIFACT=true
LIVE_OBTAIN_DOES_NOT_REWRITE_POPULATED_ORIGIN_FAIL_CLOSED=true
...
`content_bytes` through the already-bound live session. It must not invent a
...
## 4. Relationship to parent, live-session, origin, and kg-read families

- Parent reader landed ≠ official hashes attested ≠ `SOURCE_002_ROW_LEVEL_READ`
- Live session bound ≠ content bytes obtained ≠ `SOURCE_002_ROW_LEVEL_READ`
...
- This family's later unique legal flip, if any, is
  ...
  after TRAIN/VAL `content_bytes` are actually obtained through the bound live
  session — not `SOURCE_002_ROW_LEVEL_READ`
...
- Empty-table DISTINCT / empty replay-table read / zero-row live read are not
  kg read evidence, not `SOURCE_002_ROW_LEVEL_READ` evidence, and not live-obtain
...
Live §4.4 facts at base `915b625`:
...
`content_bytes` through the bound live session. That R1 still must not uniquely
...
- parent reader 已落地 ≠ 官方 hash 已从 live read attestation ≠
  `SOURCE_002_ROW_LEVEL_READ`
- live session 已绑定 ≠ TRAIN/VAL `content_bytes` 已取得 ≠
  `SOURCE_002_ROW_LEVEL_READ`
...
- 本家族是【经已绑定 live session 取得 TRAIN/VAL content_bytes】的独立 slice；
  本 freeze 仍不执行该取得
...
~~~text
WORKPAPER_PATH=docs/v0-3/s3/workpapers/s3-accepted-s2-train-val-source-002-row-level-read-live-obtain-contract.md
EVIDENCE_JSON_PATH=docs/v0-3/s3/evidence/s3-accepted-s2-train-val-source-002-row-level-read-live-obtain-contract.json
~~~

## 13. S3-A2 accepted S2 TRAIN/VAL SOURCE_002 row-level-read live-obtain contract live-authority pointer
...
## 14. S3-A2 accepted S2 TRAIN/VAL SOURCE_002 row-level-read live-obtain authorization pointer
...
## 15. S3-A2 accepted S2 TRAIN/VAL SOURCE_002 row-level-read live-obtain R1 pointer
