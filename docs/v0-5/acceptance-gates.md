...
本任务只能交付文档/模板，真实位置、天气、实验、集成依次另行实施。
...
| G1 身份/位置 | 用户或授权位置、坐标系/海拔基准、exact/授权alias、provenance/review | LOCATION_STATUS=PENDING；无真实zone/weather绑定 |
| G2 业务域 | 每farm-season 04-15前后审计、内部缺数检查、完整性/面积资格、新数据版本 | 不以部分累计量作整季标签，不改旧authority |
...
| G5 评价预注册 | origins、farm/zone留出、指标、样本门槛、候选、超参、数值gate先冻结 | 不先看成绩再降阈值/选窗口 |
...
- S1：四份authority输出（业务域、registry、zone profile、mapping）完整且可审计；
  当前 `S1_IMPLEMENTATION_STATUS=WAITING_FOR_FARM_LOCATION_INPUT`，不假装已生成。
- S2：四类天气链逐类资格、许可、可得性、版本和缺失策略齐备；7日/15日 PIT 门分开判断，
  “源有15天产品”不证明每个历史origin在当地日界下有完整15天。
- S3：A/B/C定义与各自证据分母、out-of-farm结果、选择记录；未知未登记farm fail closed。
...
| ID | 成功条件 | 可审计完成证据 |
| --- | --- | --- |
| SUCCESS_1 | 所有云南登记农场进入统一Farm Registry | 用户确认的范围清单、逐条登记/排除/待补清单、对账分母 |
| SUCCESS_2 | 所有模型农场具有可审计Climate Zone | 已审zone版本、位置溯源、mapping覆盖与例外记录 |
| SUCCESS_3 | 训练/预测统一04-15边界 | 每季边界审计、错误请求fail-closed、尾果未删未补0 |
| SUCCESS_4 | 登记农场不再普遍因缺完整同场历史不可预测 | 各等级可预测/不可预测数量及原因；覆盖门槛S1/S2后冻结 |
...
| SUCCESS_10 | 跨农场验证 | 整farm留出、全farm结果、共同可比集与覆盖扩展集 |
...
隐藏未覆盖农场、把LEVEL_C当A，或把工程CI绿色当预测精度合格。
**本规划不设WAPE<10%、peak error<3天等数值阈值**。S1/S2完成后按真实农场/zone/季节/lead
...
7. 无位置/PENDING不能调用精确天气；模板说明、空行不能导入成真实farm。
