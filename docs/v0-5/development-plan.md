...
| V0_5_GOAL_2 | 扩大多农场适用性，不再仅限完整同场历史的少数农场 |
...
| V0_5_GOAL_6 | 跨农场、跨气候带、跨产季验证；杨柳 golden 仅作工程回归 |
...
- `business_season_total_kg = sum(qualified daily harvest kg within business scope)`。
- `business_yield_kg_per_mu = business_season_total_kg / authorized productive_area_mu`。
...
S1-01 每个 farm × season 未来必须产生：`raw_first_harvest_date`、`raw_last_harvest_date`、
...
## 5. Farm Registry 与位置输入

规划 registry（未创建表、未导入真实 records）：

| 字段组 | 字段与约束 |
| --- | --- |
| 身份 | farm_id, canonical_farm_name, aliases；稳定ID，exact/授权alias，禁止 fuzzy 自动合并 |
| 行政位置 | province, prefecture, county, township；不直接充当气候带 |
| 地理 | latitude, longitude, elevation_m；用户/授权来源，附坐标系、精度、高程基准，缺失PENDING |
| 业务规模 | productive_area_mu, varieties；正finite亩数及来源，品种仅metadata，不强制拆模 |
...
位置输入规则和字段模板见 [README](README.md) 与 [CSV](farm-registry-input-template.csv)。
`latitude/longitude/elevation_m=USER_OR_AUTHORIZED_SOURCE_REQUIRED`。不从名称推断精确坐标。
历史面积按同 canonical farm 固定跨季的既有授权规则复用，保留有效scope和authority来源；
不得推导、跨场复制或把 PREVIOUS_SEASON_PROXY 自动升级。未来请求亩数不自动继承历史面积。
...
高海拔冷凉，但这些是概念，不是已确定的五个zone、边界或实测农场归属。
最终数量/边界由真实农场位置、长期气候画像、样本规模、区域代表性及稳定性审查决定。
...
`FARM_CLIMATE_ZONE_MAPPING_V1`，本轮不生成真实画像或分区结果。

## 7. 天气研究与 PIT

四类数据、候选矩阵和待验点见 [天气源评估](weather-source-evaluation.md)。
研究绝对值及相对于 farm/zone 同期正常值的 anomaly，但不假设云南所有zone共享阈值。
...
跨农场训练也不能带入任何起点之后的记录；归一化、插值策略、气候normal、特征选择在训练折内。
旧已看过窗口声明 retrospective/not blind，新留出只评一次；新数据授权不等于解除旧封存TEST。

## 8. 多农场 Applicability 与两个天气问题

| 等级 | 主要候选信息 | 限制 |
| --- | --- | --- |
| LEVEL_A | 充分farm history + climate zone + weather | 充分的数值定义在样本审计后冻结，不能仅按1季天数认定 |
| LEVEL_B | climate-zone prior + limited farm calibration + weather | 需独立验证有限历史迁移，不保证同A证据 |
...
fallback_level, limitations。目标是登记农场进入统一体系，不等于任意名称或所有登记条目立刻能出量。
`UNKNOWN_UNREGISTERED_FARM_FAIL_CLOSED=true`；禁止 unknown farm → company global mean。
未来分层prior需单独版本和审查，不能改变当前 AREA_YIELD_B1_V1 的无fallback合同。

仅规划比较：Global baseline、Climate-zone baseline、Climate-zone + farm effect、
Hierarchical/partial pooling、Mixed-effects、Regional ML + farm calibration。历史充分时
farm-specific证据权重可更高、历史少时pooled证据可更多，具体权重不得本轮预定或据已看标签调整。

S4A WEATHER-AWARE YIELD：研究 baseline yield + zone + farm effect + weather anomaly
...
| 阶段 | 冻结任务清单 | 输出与入口 |
| --- | --- | --- |
| V0.5-S1 BUSINESS_SEASON_BOUNDARY_AND_FARM_CLIMATE_FOUNDATION | S1-01 历史04-15审计；S1-02 registry contract；S1-03 位置导入/校验设计；S1-04 长期画像生成设计；S1-05 云南候选分区；S1-06 farm→zone mapping freeze | BUSINESS_SEASON_SCOPE_AUTHORITY_V1 / FARM_REGISTRY_V1 / CLIMATE_ZONE_PROFILE_V1 / FARM_CLIMATE_ZONE_MAPPING_V1；当前 WAITING_FOR_FARM_LOCATION_INPUT |
| V0.5-S2 WEATHER_DATA_AUTHORITY_AND_POINT_IN_TIME_PIPELINE | S2-01 源候选审计；S2-02 climatology链；S2-03 historical actual链；S2-04 as-issued链；S2-05 live链；S2-06 snapshot/version/hash；S2-07 缺失/延迟/fail-closed | WEATHER_SOURCE_AUTHORITY_V1 / FARM_WEATHER_SNAPSHOT_V1；需合法位置和来源许可 |
| V0.5-S3 MULTI_FARM_APPLICABILITY_BASELINE | S3-01 v0.4算法在04-15域新基线；S3-02 global pooled；S3-03 zone baseline；S3-04 partial pooling候选；S3-05 A/B/C定义；S3-06 out-of-farm验证；S3-07 selection | MULTI_FARM_APPLICABILITY_POLICY_V1 / MULTI_FARM_BASELINE_V1；需S1、合法数据与冻结评估设计 |
...
| V0.5-S6 CROSS_FARM_VALIDATION_PRODUCTIZATION_MCP_RELEASE | S6-01 cross-season；S6-02 cross-farm；S6-03 cross-zone；S6-04 A/B/C；S6-05 PIT replay；S6-06 API；S6-07 persistence；S6-08 MCP；S6-09 豆包E2E；S6-10 release资格 | 版本化产品、独立验收包、release review；无自动发版权限 |

S1部分文档可先准备，真实位置缺口仍阻止该阶段完整实施/验收；不伪报整个规划任务被阻塞。
S2官方资料桌面评估可并行，真实farm采集必须等待位置资格。阶段完成不自动授权下一阶段。

## 10. 评价设计与模型选择

必须按 BY_FARM / BY_CLIMATE_ZONE / BY_APPLICABILITY_LEVEL / BY_SEASON 分层；
报告可算/排除数量、完整季数和不同origin，不把重叠日窗当独立农场样本。
主表farm等权macro，kg加权WAPE另列诊断；分层分母、零分母不可算、缺数/censor状态透明。
...
同一合格集合必须并列 V0_4_NO_WEATHER_BASELINE、V0_5_MULTI_FARM_NO_WEATHER、
V0_5_MULTI_FARM_WEATHER_AWARE；不隐藏失利候选。v0.4算法在04-15域的复算是新实验数据，
不得冒充原v0.4已接受结果；不支持的新农场显示unsupported，不用复制别场输出填满基线。
定义共同可比集与扩展覆盖集分别报告，避免仅删掉困难农场宣称增益。

时间顺序多origin；out-of-farm整farm留出，cross-zone整zone留出；正常期、特征与模型选择
只用训练数据。新农场迁移不能见其留出标签。实验样本不足就明确证据不足，不随机拆相邻日。
...
```yaml
farm: requested_identity
canonical_farm: authorized_identity
...
V0.5 in scope：云南分区、Farm Registry、04-15边界、多农场适用性、weather authority及
weather-aware研究、7/15日运营峰值、PIT、API/MCP/豆包产品化。
另行授权才可进入：自动采摘调度、用工排班、车辆/冷库调度、生产排产、其他省份/全球泛化、
卫星遥感、病虫害视觉模型、天气商业采购合同执行。

当前必须由用户提供 canonical_farm_name/province/prefecture/county/township/latitude/
longitude/elevation_m，建议附productive_area_mu/varieties/notes。缺失可PENDING，不猜数。
