...
The canonical record field inventory shared by API and spreadsheet input is:
...
```text
sealed_batch_and_completeness_validation
-> transport_parse
...
The first failure is not the only failure: the batch may report all deterministic row errors, but no error may be silently dropped. Preview is read-only. `VALIDATED` requires a server-sealed batch, zero invalid rows, and a matching validation result hash bound to the seal manifest. Validation cannot start from `RECEIVED` or `UPLOADING`.
...
The source-time fields are `source_recorded_at`, `import_received_at`, and `ingested_at`. `source_recorded_at` is an originating-source assertion and must carry `source_recorded_at_authority_status` plus `source_recorded_at_authority_reference_or_null`. The status is one of `TRUSTED_SOURCE_TIMESTAMP`, `USER_ASSERTED_UNVERIFIED`, `MISSING`, or `CONFLICTING`. `import_received_at` and `ingested_at` are server-generated, immutable, and never accepted from API/CSV/XLSX input. User-entered time, upload time, file metadata, row order, batch order, import order, and `harvest_business_date` cannot establish historical visibility.

Historical as-of eligibility requires:
...
Untrusted, missing, or conflicting source time is not historical-as-of eligible. Such evidence may remain staging or committed and may participate in final adjudication only after separate final-eligibility gates.
...
Each logical source record uses `logical_record_key = source_system + external_logical_record_id`. Each revision uses `revision_key = source_system + external_revision_id`. Revisions use monotonically declared `revision_number` and `supersedes_external_revision_id`; initial revisions point to null and later revisions point to a concrete previous revision. The winner is the unique visible terminal node on one valid explicit chain, or the unique finalized terminal for final adjudication. Import order, latest batch, largest revision shortcut, timestamp, and database ID are forbidden.
...
same revision identity with different payload
...
An as-of snapshot may see only records with `source_recorded_at <= label_observation_cutoff_at` and `source_recorded_at_authority_status=TRUSTED_SOURCE_TIMESTAMP`, plus a valid visible lineage. `import_received_at` and `ingested_at` prove application receipt and persistence only; they never override source visibility. `revised_at` and `finalized_at` provide additional evidence when present; they never override the visibility cutoff. Future corrections must not leak into historical backtests. A missing date is never generated or treated as zero.

If the source lacks trusted `source_recorded_at`, revision identity, or terminal status, the import may remain staging/committed evidence but cannot produce a historical evaluation snapshot. Final adjudication remains a separate eligibility mode and cannot silently be treated as point-in-time replay.
...
Same key and same canonical payload returns the original result. Same key with different payload is a deterministic conflict. Same `revision_key` and canonical payload is an idempotent replay; a different payload is a revision identity conflict. Same `logical_record_key` and `revision_number` with a different `revision_key` is a revision number conflict. A revision referenced by multiple successors is a lineage conflict.
...
## 10. Revision-first canonical-grain aggregation

The target label grain is:

```text
SEASON X FARM X SUBFARM_OR_PLOT X VARIETY X HARVEST_BUSINESS_DATE
```

At the requested visibility cutoff, select one valid terminal revision per logical source record before grouping. Multiple logical records may share a grain and must not be rejected as duplicates. Group visible terminals by canonical grain and compute an exact Decimal SUM. The ordered contributing revision manifest is keyed by `source_system`, `external_logical_record_id`, and `external_revision_id`, and includes ordered hashes, count, exact sum, visibility mode, cutoff, `aggregation_policy_version`, and `aggregation_manifest_hash`. API and spreadsheet canonical records must aggregate equivalently; input order cannot affect the result.

## 11. Immutable seal and full-batch atomic commit
...
`SEALED` is the unique completeness boundary. The batch seal contains `expected_record_count_or_null`, `uploaded_record_count`, `sealed_record_count_or_null`, `sealed_at_or_null`, `sealed_by_identity_or_null`, `seal_status`, `server_raw_payload_hash_or_null`, `canonical_batch_hash_or_null`, and `seal_manifest_hash_or_null`. The server computes authoritative counts and hashes; client assertions are checked but never authoritative. After sealing, records cannot be added, removed, replaced, reordered, or modified. Parse failures cannot produce a partial sealed batch. Validation starts only from `SEALED`, and commit rechecks the unchanged seal, mapping, validation, and aggregation hashes.

Parse or validation failures use the corresponding failure status. Commit is allowed only from `VALIDATED`, with matching input, mapping, validation, and source hashes. v1 is full-batch atomic: any invalid row or concurrency conflict prevents all rows in the batch from becoming effective. Partial commit is forbidden. A failed commit must be safely retryable or fail without partial activation. Append/seal, seal/validate, validate/commit, commit/commit, and cancel/commit races fail closed.
...
## 13. Concurrency and future acceptance tests
...
## 14. Future implementation decomposition
