...
PACKAGE_DIR = Path(os.environ.get("PACKAGE_DIR", "")).resolve() if os.environ.get("PACKAGE_DIR") else None
...
git("cat-file", "-e", f"{BASE_SHA}:docs/forecast-quality/s3-round-a-authorization-package/README.md")
...
for line in sha_file.read_text(encoding="utf-8").splitlines():
    ...
    actual_hash = subprocess.check_output(
        ["git", "show", f"{BASE_SHA}:{relative_path}"], cwd=ROOT
    )
...
def read_value(value: Any, name: str, default: Any = ...,) -> Any:
...
print(f"PUBLIC_INTERNAL_REASON_CODE_DISJOINT={str(not bool(internal_reason_codes & actual_reason_codes)).lower()}")
...
if getattr(daily, "compute_metric_input_mask_hash", None) is getattr(canonical, "compute_metric_input_mask_hash", None):
...
base_exception = getattr(exceptions, "ForecastQualityError")
...
decimal_cases = {"1": "1.000000", "1.2": "1.200000", "0.000001": "0.000001", "0.0000005": "0.000000"}
...
invalid_values: list[Any] = [1.0, float("nan"), float("inf"), float("-inf"), Decimal("NaN"), Decimal("Infinity")]
...
def common_values(quantile: Any = None, forecast_business_key: str = "forecast-p50", value: str = "11.000000") -> dict[str, Any]:
    return {
        ...
        "forecast_quantile": quantile or getattr(enums.SupportedQuantile, "P50"),
        ...
        "forecast_visibility_timestamp": datetime(2025, 2, 1, tzinfo=timezone.utc),
...
registry_counts = {name: read_value(registry, name) for name in ("forecast_row_count_before", "forecast_row_count_after", "unique_actual_physical_row_count")}
if registry_counts != {"forecast_row_count_before": 3, "forecast_row_count_after": 3, "unique_actual_physical_row_count": 1}:
...
forecast_rows = []
for quantile, values in zip(quantiles, (("5.000000", "3.000000"), ("7.000000", "4.000000"), ("9.000000", "5.000000"))):
    for index, value in enumerate(values):
        forecast_rows.append(construct(schemas.S3BindingRow, common_values(quantile, f"forecast-{quantile.name}-{index}", value)))
...
expected_forecast_sums = {"P50": Decimal("8.000000"), "P80": Decimal("11.000000"), "P90": Decimal("14.000000")}
for aggregate in forecast_aggregates:
    quantile_name = member_name(read_value(aggregate, "forecast_quantile"))
    if as_decimal(read_value(aggregate, "forecast_value_kg")) != expected_forecast_sums[quantile_name]:
...
if len(actual_aggregates) != 1 or as_decimal(read_value(actual_aggregates[0], "actual_value_kg")) != Decimal("10.000000"):
...
p50_rows = []
for index, (forecast, actual) in enumerate((("11.000000", "10.000000"), ("0.000000", "0.000000"), ("8.000000", "10.000000"))):
...
version = getattr(enums.FrozenVersion, "METRIC_INPUT_MASK_V1")
evaluation_values = {
    "rows": p50_rows,
    ...
    "baseline_policy_version": getattr(enums.FrozenVersion, "NAIVE_BASELINE_POLICY_V1"),
}
...
oracles = {
    ...
    "daily_smape": (Decimal("2") / Decimal("21") + Decimal("4") / Decimal("18"), Decimal("3"), (Decimal("2") / Decimal("21") + Decimal("4") / Decimal("18")) / Decimal("3")),
...
for name, (numerator, denominator, expected) in oracles.items():
    ...
    print(f"DAILY_METRIC_ORACLE={name}|numerator={numerator}|denominator={denominator}|expected={expected_rounded}|actual={actual}|status={status_name(read_value(cell, 'metric_status'))}|reason={member_name(read_value(cell, 'reason_code'))}")
...
for axis in ("season_business_key", "farm_business_key", "subfarm_business_key", "variety_business_key", "model_identity", "forecast_horizon_days"):
    if read_value(cell_identity, axis, None) is None:
        raise AssertionError(f"breakdown axis missing: {axis}")
if status_name(read_value(cell, "metric_status")) != "INSUFFICIENT_SAMPLE" or member_name(read_value(cell, "reason_code")) != "BELOW_MINIMUM":
...
calendar_cases = [
    ("normal", date(2025, 2, 10), date(2025, 1, 1), date(2025, 3, 31), date(2024, 1, 1), date(2024, 3, 31), date(2024, 2, 10)),
    ("leap_to_leap", date(2024, 2, 29), date(2024, 1, 1), date(2024, 3, 31), date(2020, 1, 1), date(2020, 3, 31), date(2020, 2, 29)),
    ("leap_to_non_leap", date(2024, 2, 29), date(2024, 1, 1), date(2024, 3, 31), date(2023, 1, 1), date(2023, 3, 31), date(2023, 2, 28)),
    ("prior_short_overflow", date(2025, 3, 31), date(2025, 1, 1), date(2025, 3, 31), date(2024, 1, 1), date(2024, 3, 1), None),
    ("current_after_end", date(2025, 4, 1), date(2025, 1, 1), date(2025, 3, 31), date(2024, 1, 1), date(2024, 3, 31), None),
    ("first_day", date(2025, 1, 1), date(2025, 1, 1), date(2025, 3, 31), date(2024, 1, 1), date(2024, 3, 31), date(2024, 1, 1)),
    ("prior_feb_start_overflow", date(2025, 3, 31), date(2025, 1, 1), date(2025, 3, 31), date(2024, 2, 1), date(2024, 3, 31), None),
    ("mid_season", date(2025, 1, 15), date(2025, 1, 1), date(2025, 3, 31), date(2024, 1, 1), date(2024, 3, 31), date(2024, 1, 15)),
]
for case_id, target, current_start, current_end, prior_start, prior_end, expected in calendar_cases:
    actual = call_with_contract_args(calendar.resolve_prior_season_analog_date, {
    ...
    print(f"CALENDAR_CASE={case_id}|current_target_date={target}|expected={expected}|actual={actual}|result={'PASS' if actual == expected else 'FAIL'}")
...
def generic_value(name: str, overrides: Mapping[str, Any]) -> Any:
    ...
    if name.endswith("_date") or name in {"current_target_date", "prior_target_date", "current_season_start", "current_season_end", "prior_season_start", "prior_season_end"}:
    ...
    if "quantile" in name:
        return getattr(enums.SupportedQuantile, "P50")
    if "status" in name:
        return getattr(enums.MetricStatus, "COMPUTED")
    if "reason" in name:
        return getattr(enums.ReasonCode, "NONE")
    if "availability" in name:
        return getattr(enums.ComparisonAvailability, "AVAILABLE")
    if name == "baseline_policy_version":
        return getattr(enums.FrozenVersion, "NAIVE_BASELINE_POLICY_V1")
    if name == "season_mapping_policy_version":
        return getattr(enums.FrozenVersion, "SEASON_ANALOG_MAPPING_V1")
    if name == "metric_policy_version":
        return getattr(enums.FrozenVersion, "METRIC_INPUT_MASK_V1")
...
def make_baseline_request(target: date, cutoff: datetime) -> Any:
    ...
    return construct(schemas.BaselineRequest, {name: generic_value(name, overrides) for name in field_names(schemas.BaselineRequest)})


def make_baseline_snapshot(rows: list[dict[str, Any]]) -> Any:
    overrides = {"actual_rows": rows, "visibility_cutoff_at": datetime(2025, 2, 15, tzinfo=timezone.utc)}
    return construct(schemas.BaselineSourceSnapshot, {name: generic_value(name, overrides) for name in field_names(schemas.BaselineSourceSnapshot)})


def baseline_row(target_date: date, value: str = "4.000000", visibility: datetime | None = None, source_kind: str = "FARM_PICK") -> dict[str, Any]:
    return {
        "target_date": target_date,
        "actual_value_kg": Decimal(value),
        "physical_key": f"physical-{target_date}",
        "stable_actual_identity": f"actual-{target_date}",
        "visibility_timestamp": visibility or datetime(2025, 2, 1, tzinfo=timezone.utc),
...
baseline_fixtures = [
    ...
    ("Feb29_to_Feb28", date(2024, 2, 29), [baseline_row(date(2023, 2, 28), visibility=datetime(2024, 2, 1, tzinfo=timezone.utc))], "COMPUTED", "NONE"),
    ("no_analog_day", date(2025, 3, 31), [baseline_row(date(2024, 3, 1))], "NOT_COMPUTABLE", "NO_PRIOR_SEASON_ANALOG_DAY"),
    ("no_analog_actual", date(2025, 2, 10), [], "NOT_COMPUTABLE", "NO_PRIOR_SEASON_ANALOG_ACTUAL"),
    ("visible_at_current_cutoff", date(2025, 2, 10), [baseline_row(date(2024, 2, 10), visibility=datetime(2025, 2, 1, tzinfo=timezone.utc))], "COMPUTED", "NONE"),
    ("late_revision_not_visible", date(2025, 2, 10), [baseline_row(date(2024, 2, 10), visibility=datetime(2025, 2, 20, tzinfo=timezone.utc))], "NOT_COMPUTABLE", "BASELINE_SOURCE_NOT_VISIBLE_AT_CURRENT_FORECAST_CUTOFF"),
]
...
for fixture_id, target, fixture_rows, expected_status, expected_reason in baseline_fixtures:
    result = baseline.resolve_baseline_point_forecast(make_baseline_request(target, datetime(2025, 2, 15, tzinfo=timezone.utc)), make_baseline_snapshot(fixture_rows))
    actual_status = status_name(read_value(result, "metric_status", read_value(result, "status", None)))
...
red_source_cases = ["latest_actual_fallback", "post_cutoff_revision", "model_forecast_proxy", "receipt_arrival_proxy", "implicit_zero_fallback", "s2_row_set_reuse"]
for case_id in red_source_cases:
    row = baseline_row(date(2024, 2, 10), source_kind=case_id)
    result = baseline.resolve_baseline_point_forecast(make_baseline_request(date(2025, 2, 10), datetime(2025, 2, 15, tzinfo=timezone.utc)), make_baseline_snapshot([row]))
    actual_status = status_name(read_value(result, "metric_status", read_value(result, "status", None)))
...
point_result = baseline.resolve_baseline_point_forecast(make_baseline_request(date(2025, 2, 10), datetime(2025, 2, 15, tzinfo=timezone.utc)), make_baseline_snapshot([baseline_row(date(2024, 2, 10))]))
if read_value(point_result, "baseline_quantile", "P50") not in {"P50", getattr(enums.SupportedQuantile, "P50")}:  # type: ignore[operator]
...
expected_root_fields = {
    "schema_version", "s2_run_identity", "s2_manifest_identity", "s2_binding_row_set_hash", "baseline_source_snapshot_identity", "baseline_source_snapshot_hash", "baseline_source_row_set_hash", "baseline_source_visibility_manifest_hash", "baseline_source_visibility_cutoff_at", "baseline_policy_version", "season_analog_mapping_policy_version", "prior_season_identity", "baseline_grain", "baseline_horizon_rule", "breakdown_dimensions", "s2_total_row_count", "s2_comparable_row_count", "s2_excluded_row_count", "s2_not_computable_row_count", "coverage_ratio", "metric_input_mask_policy_version", "metric_input_mask_hash", "metric_input_row_count", "metric_input_quantile", "unique_actual_physical_row_count", "per_breakdown_cell",
}
expected_cell_fields = {
    "breakdown_identity", "metric_input_row_count", "eligible_row_count", "excluded_row_count", "not_computable_row_count", "coverage_ratio", "metric_input_mask_policy_version", "metric_input_mask_hash", "metric_input_quantile", "unique_actual_physical_row_count", "daily_mae", "daily_wape", "daily_smape", "daily_mape", "daily_bias_kg",
}
root_payload = invoke_one_argument(canonical.build_baseline_canonical_payload_root, baseline_result_for_canonical)
cell_payload = invoke_one_argument(canonical.build_baseline_canonical_payload_cell, baseline_result_for_canonical)
if set(root_payload) != expected_root_fields:
    raise AssertionError(f"baseline root field set mismatch: {sorted(set(root_payload) ^ expected_root_fields)}")
if set(cell_payload) != expected_cell_fields:
    raise AssertionError(f"baseline cell field set mismatch: {sorted(set(cell_payload) ^ expected_cell_fields)}")
