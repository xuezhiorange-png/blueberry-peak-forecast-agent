...
Round 6 additionally binds every gate's `PACKAGE_DIR` to the canonical
`ROUND_A_WORKTREE/docs/forecast-quality/s3-round-a-authorization-package`
directory. External copies and symlink escapes fail closed before any package
manifest or hash is read.
...
The 41 source-map records are executable comparisons, not field-name
templates: each record names an existing schema or frozen constant, reads its
value from the corresponding source object, normalizes it, and compares it to
the canonical payload. Six valid null states and five invalid null states are
counted separately, with root and cell builders exercised independently.
