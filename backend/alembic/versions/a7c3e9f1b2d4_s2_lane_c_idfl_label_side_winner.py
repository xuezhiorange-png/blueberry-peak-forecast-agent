"""Create append-only S2 Lane C IDFL label-side winner decision table.
...
IDFL_LABEL_SIDE_MODE_VALUES = ("IDFL_LABEL_SIDE",)
...
def upgrade() -> None:
    op.create_table(
        "s2_idfl_label_side_winner_decision",
        ...
        sa.Column("visibility_boundary", sa.Text(), nullable=False),
        ...
        sa.Column("no_winner_reason", sa.Text(), nullable=False),
        ...
        sa.CheckConstraint(
            "revision_number >= 1",
            name="ck_s2_idfl_label_side_winner_revision_number",
        ),
        sa.CheckConstraint(
            _enum_check("mode", IDFL_LABEL_SIDE_MODE_VALUES),
    ...
    _create_lane_c_immutability_guard(
        "s2_idfl_label_side_winner_decision",
        "s2 idfl label side winner decision is immutable",
    )


def downgrade() -> None:
    _drop_lane_c_immutability_guard("s2_idfl_label_side_winner_decision")
    op.drop_table("s2_idfl_label_side_winner_decision")
