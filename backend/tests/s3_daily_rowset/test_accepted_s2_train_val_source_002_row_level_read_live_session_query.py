"""S3-A2 accepted S2 TRAIN/VALIDATION SOURCE_002 row-level-read live-session-query tests."""
...
AcceptedS2TrainValLiveSessionQueryEnvelope = _query.AcceptedS2TrainValLiveSessionQueryEnvelope
...
def test_unreadable_session_provider_fail_closes() -> None:
    ...
    assert envelope.queryable is False
    assert envelope.reason_code is LiveSessionQueryReasonCode.FAIL_CLOSED_SESSION_UNREADABLE
    _assert_not_source_002(envelope)


def test_synthetic_sqlite_queryable_is_not_official_live_queryable() -> None:
    ...
    assert envelope.reason_code is LiveSessionQueryReasonCode.QUERYABLE
    _assert_not_source_002(envelope)


def test_query_envelope_does_not_expose_content_bytes_or_kg() -> None:
    field_names = set(AcceptedS2TrainValLiveSessionQueryEnvelope.model_fields)
...
def test_bound_live_session_query_fail_closed_is_not_source_002_row_level_read() -> None:
    bind_default_source_002_row_level_read_live_session_provider()
    ...
    if envelope.queryable:
        ...
        assert envelope.reason_code is not LiveSessionQueryReasonCode.QUERYABLE
    _assert_not_source_002(envelope)
