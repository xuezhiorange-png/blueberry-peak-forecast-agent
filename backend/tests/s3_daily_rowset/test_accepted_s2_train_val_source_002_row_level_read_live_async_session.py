"""S3-A2 accepted S2 TRAIN/VALIDATION SOURCE_002 row-level-read live-async-session tests."""
...
AcceptedS2TrainValLiveAsyncSessionEnvelope = (
    _async_session.AcceptedS2TrainValLiveAsyncSessionEnvelope
)
...
LIVE_ASYNC_CONNECTION_TEST_PY_BLOB = "ae84ec37387a71c9180ded500271c368840cedd8"
...
def test_official_live_async_session_maker_path_fail_closed_or_obtained() -> None:
    envelope = obtain_accepted_s2_train_val_async_session_from_the_already_configured_live_async_sessionmaker()  # noqa: E501

    if envelope.obtained:
        assert envelope.reason_code is LiveAsyncSessionReasonCode.OBTAINED
    else:
        assert envelope.reason_code is not LiveAsyncSessionReasonCode.OBTAINED
    _assert_not_source_002(envelope)


def test_synthetic_sqlite_aiosqlite_obtained_is_not_official_live_async_session() -> None:
...
def test_missing_async_session_maker_fail_closes_no_async_session_maker() -> None:
    ...
    assert envelope.reason_code is LiveAsyncSessionReasonCode.FAIL_CLOSED_NO_ASYNC_SESSION_MAKER
    _assert_not_source_002(envelope)
...
def test_session_maker_that_raises_on_enter_fail_closes_not_obtained() -> None:
    ...
    assert (
        ...
        is LiveAsyncSessionReasonCode.FAIL_CLOSED_ASYNC_SESSION_NOT_OBTAINED_FROM_SESSION_MAKER
    )
    _assert_not_source_002(envelope)


def test_session_envelope_does_not_expose_content_bytes_or_kg() -> None:
    field_names = set(AcceptedS2TrainValLiveAsyncSessionEnvelope.model_fields)
...
def test_sibling_query_obtain_live_connection_and_async_connection_still_not_source_002() -> None:
    bind_default_source_002_row_level_read_live_session_provider()
    ...
    async_connection_envelope = obtain_accepted_s2_train_val_async_connection_from_the_already_configured_live_async_engine()  # noqa: E501
    ...
    if async_connection_envelope.connected:
        assert async_connection_envelope.reason_code is LiveAsyncConnectionReasonCode.CONNECTED
    else:
        assert async_connection_envelope.reason_code is not LiveAsyncConnectionReasonCode.CONNECTED
    ...
    _assert_not_source_002(async_connection_envelope)
...
def test_async_session_module_contains_no_forbidden_patterns() -> None:
    source = LIVE_ASYNC_SESSION_MODULE.read_text(encoding="utf-8").lower()
...
def test_parent_reader_live_session_obtain_query_live_connection_async_connection_and_session_blobs_unchanged() -> (  # noqa: E501
    None
):
