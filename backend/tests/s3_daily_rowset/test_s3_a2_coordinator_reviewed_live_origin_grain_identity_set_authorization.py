"""S3-A2 coordinator-reviewed live-origin grain identity-set authorization tests."""
...
UNIQUE_FLIP = "S3_A2_COORDINATOR_REVIEWED_LIVE_ORIGIN_GRAIN_IDENTITY_SET_IMPLEMENTATION_AUTHORIZED"
...
def test_grant_unique_flip_is_implementation_authorized() -> None:
    ...
    assert flags["S3_A2_COORDINATOR_REVIEWED_LIVE_ORIGIN_GRAIN_IDENTITY_SET_CONTRACT_AUTHORIZED"]
    assert flags[
        "DETERMINISTIC_COORDINATOR_REVIEWED_LIVE_ORIGIN_GRAIN_IDENTITY_SET_IMPLEMENTED"
    ] is (False)
    assert flags["DETERMINISTIC_COMPLETENESS_PASS_CLOSEOUT_IMPLEMENTED"] is True
...
def test_grant_does_not_flip_completeness_or_invent_weather_plans() -> None:
    ...
    assert flags["LATER_R1_MAY_LAND_THESE_THREE_POLICY_GRAINS_AS_REVIEWED_SET"] is True
...
def test_grant_pointers_are_appended_not_rewritten() -> None:
    ...
    assert "DETERMINISTIC_COMPLETENESS_PASS_CLOSEOUT_IMPLEMENTED=true" in live_intro
    assert "s3-a2-coordinator-reviewed-live-origin-grain-identity-set-authorization.md" in plan
    ...
    r1_snapshot = amendment.split("## 187.", 1)[1].split("## 188.", 1)[0]
    assert "DETERMINISTIC_COMPLETENESS_PASS_CLOSEOUT_IMPLEMENTED=true" in r1_snapshot
    contract_pointer = plan.split("### 4.5", maxsplit=1)[0]
    assert "s3-a2-coordinator-reviewed-live-origin-grain-identity-set-authorization.md" in (
        contract_pointer
    )
    payload = json.loads(GRANT_EVIDENCE.read_text(encoding="utf-8"))
    assert (
...
def test_parent_contract_commit_is_named_for_traceability() -> None:
    ...
    assert (
        payload["parent_completeness_pass_closeout_r1_commit"]
        == PARENT_COMPLETENESS_PASS_CLOSEOUT_R1_COMMIT
    )
    assert (
        payload["parent_completeness_pass_closeout_r1_evidence_json_sha256"]
        == PARENT_COMPLETENESS_PASS_CLOSEOUT_R1_EVIDENCE_JSON_SHA256
    )
