"""S3-A2 incumbent forecast artifact no-versioned flip authorization tests."""
...
PARENT_INDEPENDENT_REVIEW_GRANT_WORKPAPER = Path(
    "docs/v0-3/s3/workpapers/"
    "s3-a2-incumbent-forecast-artifact-presence-package-independent-review-authorization.md"
)
PARENT_INDEPENDENT_REVIEW_GRANT_EVIDENCE = Path(
    "docs/v0-3/s3/evidence/"
...
UNIQUE_FLIP = "S3_A2_INCUMBENT_FORECAST_ARTIFACT_NO_VERSIONED_FLIP_IMPLEMENTATION_AUTHORIZED"
...
CONTRACT_POINTER_HEADING = "#### Incumbent forecast artifact no-versioned flip contract pointer"
...
def test_grant_unique_flip_is_no_versioned_flip_implementation_authorized() -> None:
    ...
    assert flags["LATER_R1_MAY_FLIP_NO_VERSIONED"] is True
...
def test_parent_contract_blobs_and_evidence_remain() -> None:
    ...
    assert parent["no_versioned_flip_preconditions"]["no_versioned_flip_precondition_4_holds"]
...
def test_parent_independent_review_and_presence_pins_remain() -> None:
    ...
    assert _git_blob(PARENT_INDEPENDENT_REVIEW_GRANT_WORKPAPER) == (
        "54832aaa1589eb11ef6331f484c777a8c12ef58c"
    )
    assert _git_blob(PARENT_INDEPENDENT_REVIEW_GRANT_EVIDENCE) == (
        "f53083a4d79f3cfddfee296bce4ca1233324f14e"
    )
...
def test_no_versioned_flip_module_is_filename_only_and_pep420_holds() -> None:
...
def test_independent_review_records_digest_and_precondition_4_after_grant() -> None:
...
def test_grant_files_exist_and_avoid_forbidden_tokens() -> None:
    ...
    assert "LATER_R1_MAY_FLIP_NO_VERSIONED=true" in workpaper
...
def test_grant_pointers_are_appended_not_rewritten() -> None:
    ...
    assert "s3-a2-incumbent-forecast-artifact-no-versioned-flip-authorization.md" in plan
    assert "## 205." in amendment
    assert "## 206." in amendment
    assert amendment.count(SECTION_205_HEADING) == 1
    ...
    assert "NO_VERSIONED_FLIP_PRECONDITION_4_HOLDS=true" in grant_snapshot
    assert "LATER_R1_MAY_FLIP_NO_VERSIONED=true" in grant_snapshot
    assert CONTENT_IDENTITY_SHA256 in grant_snapshot
    assert REVIEW_EVIDENCE_DIGEST_SHA256 in grant_snapshot
    contract_snapshot = amendment.split("## 205.", 1)[1]
    if "## 206." in contract_snapshot:
        contract_snapshot = contract_snapshot.split("## 206.", 1)[0]
...
def test_grant_package_is_docs_only() -> None:
    ...
    assert payload["authorization_merge"]["does_not_implement_no_versioned_flip"] is True
    assert payload["authorization_merge"]["does_not_add_no_versioned_flip_module"] is True
    assert INDEPENDENT_REVIEW_MODULE.is_file()
    assert CONTENT_FOR_REVIEWED_GRAINS_MODULE.is_file()
    assert LANDING_MODULE.is_file()
    assert COMPLETENESS_PASS_CLOSEOUT_MODULE.is_file()
