"""S3-A2 completeness PASS observation R1 tests."""
...
R1_WORKPAPER = Path("docs/v0-3/s3/workpapers/s3-a2-completeness-pass-observation-r1.md")
R1_EVIDENCE = Path("docs/v0-3/s3/evidence/s3-a2-completeness-pass-observation-r1.json")
...
PARENT_GRANT_WORKPAPER_BLOB = "a7fd9b34b25223c1ee2c867c941102ba9d950b5d"
PARENT_GRANT_EVIDENCE_BLOB = "b3787e5f6f9194fa26392dc58c412badb33caeb6"
...
UNIQUE_FLIP = "DETERMINISTIC_COMPLETENESS_PASS_OBSERVATION_IMPLEMENTED"
...
def test_fail_closed_when_observation_fail_closes() -> None:
    classifier = CompletenessPassObservationClassifier()
    with patch(
        "backend.app.s3_daily_rowset."
...
def test_parent_grant_pins_remain() -> None:
    ...
    assert payload["flags"]["S3_A2_COMPLETENESS_PASS_OBSERVATION_IMPLEMENTATION_AUTHORIZED"]
    ...
    assert r1["parent_observation_r1_pr"] == PARENT_OBSERVATION_R1_PR
    assert r1["parent_observation_r1_commit"] == PARENT_OBSERVATION_R1_COMMIT
    assert r1["parent_observation_r1_merge"] == PARENT_OBSERVATION_R1_MERGE
    assert (
        r1["parent_observation_r1_evidence_json_sha256"]
        == PARENT_OBSERVATION_R1_EVIDENCE_JSON_SHA256
    )
    assert r1["flags"][UNIQUE_FLIP] is True
    assert r1["flags"]["S3_A2_COMPLETENESS_PASS_OBSERVATION_CONTRACT_AUTHORIZED"]
    assert r1["flags"]["S3_A2_COMPLETENESS_PASS_OBSERVATION_IMPLEMENTATION_AUTHORIZED"]
...
def test_r1_pointers_are_appended_not_rewritten() -> None:
    ...
    assert "S3_A2_COMPLETENESS_PASS_OBSERVATION_IMPLEMENTATION_AUTHORIZED=true" in live_intro
    assert (
    ...
    assert "s3-a2-completeness-pass-observation-r1.md" in plan
    assert "## 193." in amendment
    assert "## 194." in amendment
    assert "## 195." in amendment
    ...
    assert "S3_A2_COMPLETENESS_PASS_OBSERVATION_IMPLEMENTATION_AUTHORIZED=true" in grant_snapshot
    ...
    r1_snapshot = amendment.split("## 195.", 1)[1]
    if "## 196." in r1_snapshot:
        r1_snapshot = r1_snapshot.split("## 196.", 1)[0]
    ...
    assert (
        "S3_A2_COMPLETENESS_PASS_OBSERVATION_IMPLEMENTATION_AUTHORIZED=false" in contract_snapshot
    )
    ...
    assert "s3-a2-completeness-pass-observation-r1.md" in contract_pointer
