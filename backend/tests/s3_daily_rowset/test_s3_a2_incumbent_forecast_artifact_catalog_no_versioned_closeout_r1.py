"""S3-A2 incumbent forecast artifact catalog no-versioned closeout R1 tests."""
...
from backend.app.s3_daily_rowset import (
    s3_a2_incumbent_forecast_artifact_catalog_no_versioned_closeout as catalog_closeout,
)
from backend.app.s3_daily_rowset import (
...
IncumbentForecastArtifactCatalogNoVersionedCloseoutReasonCode = (
    catalog_closeout.IncumbentForecastArtifactCatalogNoVersionedCloseoutReasonCode
)
...
GRANT_TEST = Path(
    "backend/tests/s3_daily_rowset/"
    "test_s3_a2_incumbent_forecast_artifact_catalog_no_versioned_closeout_authorization.py"
)
...
R1_WORKPAPER = Path(
    "docs/v0-3/s3/workpapers/s3-a2-incumbent-forecast-artifact-catalog-no-versioned-closeout-r1.md"
)
R1_EVIDENCE = Path(
    "docs/v0-3/s3/evidence/s3-a2-incumbent-forecast-artifact-catalog-no-versioned-closeout-r1.json"
)
...
PARENT_GRANT_TEST_BLOB = "c24c748476b8b98e31d57e3e9b30998cd4343d86"
...
BASE_MAIN_SHA = "92a7c8622205ecf91105d307cc7e294e920ed34e"
BASE_MAIN_TREE_SHA = "933d645b58e96eb4d4533adf07b849a93eabff08"
PARENT_GRANT_PR = 523
PARENT_GRANT_MERGE = "92a7c8622205ecf91105d307cc7e294e920ed34e"
PARENT_GRANT_COMMIT = "19cf8297f41aec3443b66dc9b1e5182e3c9da39a"
PARENT_GRANT_EVIDENCE_JSON_SHA256 = (
    "130e7458fd08bf4e6b6ac1caa25194072fa042d0ba846c0d9ef9c4f27288dc4b"
)
...
UNIQUE_FLIP = "DETERMINISTIC_INCUMBENT_FORECAST_ARTIFACT_CATALOG_NO_VERSIONED_CLOSEOUT_IMPLEMENTED"
...
R1_POINTER_HEADING = "#### Incumbent forecast artifact catalog no-versioned closeout R1 pointer"
SECTION_209_HEADING = (
...
def test_production_module_does_not_land_or_embed_connection_strings() -> None:
    ...
    assert "LATER_R1_MAY_FLIP_NO_VERSIONED" not in source
    assert "EvaluationInstanceCatalogArtifactProductionService" in source
...
def test_frozen_blobs_unchanged() -> None:
    ...
    assert (
        _git_blob(PARENT_NO_VERSIONED_FLIP_R1_WORKPAPER)
        == PARENT_NO_VERSIONED_FLIP_R1_WORKPAPER_BLOB
    )
    assert (
        _git_blob(PARENT_NO_VERSIONED_FLIP_R1_EVIDENCE) == PARENT_NO_VERSIONED_FLIP_R1_EVIDENCE_BLOB
    )
...
def test_classify_records_catalog_no_versioned_closeout_on_parent_success() -> None:
    _assert_harvest_replay_and_provider_remain_empty()
    ...
    assert result.catalog_produce_reason_code is (
    ...
    assert result.this_r1_records_catalog_still_fail_closes_no_versioned is True
    assert result.this_r1_does_not_flip_no_versioned is True
    assert result.this_r1_must_not_flip_live_compact_no_versioned is True
    assert result.this_r1_must_not_make_default_catalog_produce_succeed is True
...
def test_digest_and_content_identity_inherited_from_parent_not_recomputed() -> None:
    parent = IncumbentForecastArtifactNoVersionedFlipClassifier().classify()
...
def test_fail_closed_when_origin_entries_empty_or_extra() -> None:
    classifier = IncumbentForecastArtifactCatalogNoVersionedCloseoutClassifier()
    with patch(
        "backend.app.s3_daily_rowset."
    ...
    assert empty.closeout_recorded is False
    assert empty.catalog_produce_still_fail_closes_no_versioned is False
    ...
    assert mismatched.closeout_recorded is False
    assert mismatched.catalog_produce_still_fail_closes_no_versioned is False
...
def test_fail_closed_when_content_producer_returns_none() -> None:
    ...
    assert result.catalog_produce_still_fail_closes_no_versioned is False
...
def test_frozen_independent_review_still_reports_no_versioned_true_after_closeout() -> None:
    with patch("backend.app.db.session.AsyncSessionMaker", None):
        IncumbentForecastArtifactCatalogNoVersionedCloseoutClassifier().classify()
...
def test_r1_pointer_isolation_grant_snapshot_still_implemented_false() -> None:
    ...
    assert "THIS_R1_RECORDS_CATALOG_STILL_FAIL_CLOSES_NO_VERSIONED=true" in r1_pointer
...
def test_r1_docs_avoid_forbidden_tokens() -> None:
    ...
    assert "THIS_R1_RECORDS_CATALOG_STILL_FAIL_CLOSES_NO_VERSIONED=true" in workpaper
    ...
    assert "LATER_R1_MAY_FLIP_NO_VERSIONED" not in workpaper
    evidence = R1_EVIDENCE.read_text(encoding="utf-8")
    assert "LATER_R1_MUST_NOT_FLIP_NO_VERSIONED" not in evidence
    assert "LATER_R1_MAY_FLIP_NO_VERSIONED" not in evidence
...
def test_parent_grant_pins_remain() -> None:
    ...
    assert r1["flags"]["THIS_R1_RECORDS_CATALOG_STILL_FAIL_CLOSES_NO_VERSIONED"] is True
    assert r1["flags"]["THIS_R1_DOES_NOT_FLIP_NO_VERSIONED"] is True
    assert r1["flags"]["THIS_R1_MUST_NOT_FLIP_LIVE_COMPACT_NO_VERSIONED"] is True
    assert r1["flags"]["THIS_R1_MUST_NOT_MAKE_DEFAULT_CATALOG_PRODUCE_SUCCEED"] is True
