"""S3-A2 evaluation-instance-registry AVAILABLE-closeout authorization tests."""
...
BINDABLE_REPOSITORY_CONTRACT = (
    REPO_ROOT / "docs/v0-3/s3/s3-default-catalog-bindable-repository-contract.md"
)
...
UNIQUE_FLIP = "S3_A2_EVALUATION_INSTANCE_REGISTRY_AVAILABLE_CLOSEOUT_IMPLEMENTATION_AUTHORIZED"
...
def test_grant_unique_flip_is_implementation_authorized() -> None:
    ...
    assert flags["S3_A2_EVALUATION_INSTANCE_REGISTRY_AVAILABLE_CLOSEOUT_CONTRACT_AUTHORIZED"]
    assert flags["DETERMINISTIC_EVALUATION_INSTANCE_REGISTRY_AVAILABLE_CLOSEOUT_IMPLEMENTED"] is (
        False
    )
    assert flags["DETERMINISTIC_DEFAULT_CATALOG_BINDABLE_REPOSITORY_IMPLEMENTED"] is True
...
def test_parent_presence_and_contract_pins_remain() -> None:
    ...
    assert _git_blob(BINDABLE_REPOSITORY_CONTRACT) == PARENT_BINDABLE_REPOSITORY_CONTRACT_BLOB
...
def test_fail_closed_produce_still_records_registry_unavailable_after_grant() -> None:
    clear_v0_2_live_postgres_session_provider()
...
def test_grant_pointers_are_appended_not_rewritten() -> None:
    ...
    assert "s3-a2-evaluation-instance-registry-available-closeout-authorization.md" in plan
    ...
    contract_snapshot = amendment.split("## 178.", 1)[1].split("## 179.", 1)[0]
    assert (
    ...
    assert "s3-a2-evaluation-instance-registry-available-closeout-authorization.md" in (
        contract_pointer
    )
    payload = json.loads(GRANT_EVIDENCE.read_text(encoding="utf-8"))
    assert (
...
def test_parent_contract_commit_is_named_for_traceability() -> None:
    ...
    assert payload["parent_bindable_repository_r1_merge"] == PARENT_BINDABLE_REPOSITORY_R1_MERGE
    assert (
        payload["parent_bindable_repository_r1_evidence_json_sha256"]
        == PARENT_BINDABLE_REPOSITORY_R1_EVIDENCE_JSON_SHA256
    )
