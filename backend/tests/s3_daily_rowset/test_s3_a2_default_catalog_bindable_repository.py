"""S3-A2 default catalog bindable-repository R1 tests."""
...
CLASSIFIER_MODULE = Path("backend/app/s3_daily_rowset/s3_a2_default_catalog_bindable_repository.py")
...
GRANT_WORKPAPER = Path(
    "docs/v0-3/s3/workpapers/s3-a2-default-catalog-bindable-repository-authorization.md"
)
GRANT_EVIDENCE = Path(
    "docs/v0-3/s3/evidence/s3-a2-default-catalog-bindable-repository-authorization.json"
)
R1_WORKPAPER = Path("docs/v0-3/s3/workpapers/s3-a2-default-catalog-bindable-repository-r1.md")
R1_EVIDENCE = Path("docs/v0-3/s3/evidence/s3-a2-default-catalog-bindable-repository-r1.json")
...
PARENT_GRANT_EVIDENCE_JSON_SHA256 = (
    "6120401cd50734bd732116a0a27a231ca01fdfecf22f45b40356b917eb47cdff"
)
PARENT_GRANT_MERGE = "08831b6ee5b147d203d3a15e92fc1d5f884d3509"
PARENT_GRANT_COMMIT = "360969ccda02237534431054aefeabc57d192675"
...
PARENT_GRANT_WORKPAPER_BLOB = "f3ffc98261383d8ff0ad5ec423bf203eb9a6ce29"
PARENT_GRANT_EVIDENCE_BLOB = "31f3852e70a3dcb609aac339d37cade70a9ac107"
UNIQUE_FLIP = "DETERMINISTIC_DEFAULT_CATALOG_BINDABLE_REPOSITORY_IMPLEMENTED"
...
def test_fail_closed_when_session_maker_unavailable() -> None:
    ...
    with patch("backend.app.db.session.AsyncSessionMaker", None):
        result = DefaultCatalogBindableRepositoryClassifier().classify()
    assert result.reason_code is BindableRepositoryReasonCode.CATALOG_NOT_PRODUCED
...
def test_patched_session_maker_classifies_produced_catalog_not_bindable(
    ...
    assert result.reason_code is BindableRepositoryReasonCode.NOT_BINDABLE
...
def test_parent_grant_pins_remain() -> None:
    ...
    assert payload["flags"]["S3_A2_DEFAULT_CATALOG_BINDABLE_REPOSITORY_IMPLEMENTATION_AUTHORIZED"]
    ...
    assert r1["flags"]["S3_A2_DEFAULT_CATALOG_BINDABLE_REPOSITORY_IMPLEMENTATION_AUTHORIZED"]
...
def test_r1_pointers_are_appended_not_rewritten() -> None:
    ...
    assert "S3_A2_DEFAULT_CATALOG_BINDABLE_REPOSITORY_IMPLEMENTATION_AUTHORIZED=true" in live_intro
    assert "NO_BINDABLE_CATALOG_IN_REPOSITORY=true" in live_intro
    assert "s3-a2-default-catalog-bindable-repository-r1.md" in plan
    assert "## 177." in amendment
    ...
    grant_snapshot = amendment.split("## 176.", 1)[1].split("## 177.", 1)[0]
    assert UNIQUE_FLIP + "=false" in grant_snapshot
    contract_pointer = plan.split("### 4.5", maxsplit=1)[0]
    assert "s3-a2-default-catalog-bindable-repository-r1.md" in contract_pointer
...
def test_official_live_bindable_repository_fail_closed_or_not_bindable() -> None:
    script = """
import json
from backend.app.s3_daily_rowset.incumbent_forecast_replay_source import (
    IncumbentForecastReplaySource,
...
"""
    ...
    if payload["reason_code"] == "NOT_BINDABLE":
