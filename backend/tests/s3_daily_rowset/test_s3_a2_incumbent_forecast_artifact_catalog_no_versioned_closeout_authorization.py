"""S3-A2 incumbent forecast artifact catalog no-versioned closeout authorization tests."""
...
CONTRACT_TEST = Path(
    ...
    "test_s3_a2_incumbent_forecast_artifact_catalog_no_versioned_closeout_contract.py"
)
GRANT_WORKPAPER = Path(
    "docs/v0-3/s3/workpapers/"
    "s3-a2-incumbent-forecast-artifact-catalog-no-versioned-closeout-authorization.md"
)
GRANT_EVIDENCE = Path(
    "docs/v0-3/s3/evidence/"
    "s3-a2-incumbent-forecast-artifact-catalog-no-versioned-closeout-authorization.json"
)
...
PARENT_CONTRACT_TEST_BLOB = "a407032430cb8866be5de13220464768366fca0c"
...
PARENT_NO_VERSIONED_FLIP_GRANT_EVIDENCE_JSON_SHA256 = (
...
PARENT_NO_VERSIONED_FLIP_CONTRACT_EVIDENCE_JSON_SHA256 = (
...
GRANT_POINTER_HEADING = (
    "#### Incumbent forecast artifact catalog no-versioned closeout "
...
SECTION_209_HEADING = (
    "## 209. Incumbent forecast artifact catalog no-versioned closeout "
    "implementation authorization pointer"
)
...
def test_grant_unique_flip_is_catalog_closeout_implementation_authorized() -> None:
    ...
    assert flags[PARENT_NO_VERSIONED_FLIP_IMPLEMENTED]
    assert flags[
    ...
    assert flags["NO_VERSIONED_FLIP_PRECONDITION_4_HOLDS"] is True
    assert flags["LATER_R1_MAY_RECORD_CATALOG_STILL_FAIL_CLOSES_NO_VERSIONED"] is True
    assert flags["LATER_R1_MUST_NOT_FLIP_LIVE_COMPACT_NO_VERSIONED"] is True
    assert flags["LATER_R1_MUST_NOT_MAKE_DEFAULT_CATALOG_PRODUCE_SUCCEED"] is True
    assert flags["THIS_GRANT_DOES_NOT_FLIP_NO_VERSIONED"] is True
    assert flags["GRANT_MERGE_DOES_NOT_FLIP_NO_VERSIONED"] is True
    assert flags["FROZEN_LIVE_COMPACT_NO_VERSIONED_REMAINS_TRUE"] is True
    assert flags["FROZEN_INDEPENDENT_REVIEW_STILL_REPORTS_NO_VERSIONED_TRUE"] is True
    assert flags["CATALOG_PRODUCE_STILL_FAIL_CLOSES_NO_VERSIONED"] is True
    assert flags["CONTENT_PRODUCER_ON_EMPTY_OBTAIN_RETURNS_NONE"] is True
    assert flags["THIS_R1_CLASSIFIER_FLIPS_NO_VERSIONED_ON_INDEPENDENT_REVIEW_SUCCESS"] is True
    assert "LATER_R1_MAY_FLIP_NO_VERSIONED" not in flags
    assert "LATER_R1_MUST_NOT_FLIP_NO_VERSIONED" not in flags
...
def test_parent_contract_blobs_and_evidence_remain() -> None:
    ...
    assert parent["authorization"]["catalog_produce_still_fail_closes_no_versioned"]


def test_parent_independent_review_and_presence_pins_remain() -> None:
    payload = json.loads(GRANT_EVIDENCE.read_text(encoding="utf-8"))
    ...
    assert (
        ...
        == "4c48d14fc6321313809f43505cd40812dc3ea320"
    )
    assert (
        _git_blob(PARENT_INDEPENDENT_REVIEW_R1_EVIDENCE)
        == "b29e3179e7ea319a74fa008d1cd26c541f79c1d2"
    )
    ...
    assert _git_blob(PARENT_NO_VERSIONED_FLIP_GRANT_WORKPAPER) == (
        "d440b324d47e2200cbde86f2120419a0ba9c6d62"
    )
    assert _git_blob(PARENT_NO_VERSIONED_FLIP_GRANT_EVIDENCE) == (
        "59bd88a9e899a614c36f4aeb618f75e15f94ee5b"
    )
...
def test_frozen_independent_review_still_reports_no_versioned_true_after_grant() -> None:
...
def test_frozen_content_for_reviewed_grains_still_reports_precondition_4_false() -> None:
...
def test_grant_does_not_flip_completeness_or_invent_weather_plans() -> None:
    ...
    assert flags["FROZEN_REVIEWED_SET_CLOSEOUT_STILL_REPORTS_NO_REVIEWED"] is True
    assert flags["FROZEN_COMPLETENESS_PASS_CLOSEOUT_STILL_UNAUTHORIZED"] is True
    assert flags["FROZEN_PRESENCE_R1_STILL_REPORTS_FAIL_CLOSED_NO_REVIEWED_SET"] is True
    ...
    assert flags["GRANT_MERGE_DOES_NOT_IMPLEMENT_CATALOG_NO_VERSIONED_CLOSEOUT"] is True
    assert "LATER_R1_MAY_FLIP_NO_VERSIONED" not in flags
    assert "LATER_R1_MUST_NOT_FLIP_NO_VERSIONED" not in flags
...
def test_grant_files_exist_and_avoid_forbidden_tokens() -> None:
    ...
    assert "LATER_R1_MAY_FLIP_NO_VERSIONED" not in text
    assert "LATER_R1_MUST_NOT_FLIP_NO_VERSIONED" not in text
    ...
    assert "LATER_R1_MAY_RECORD_CATALOG_STILL_FAIL_CLOSES_NO_VERSIONED=true" in workpaper
    assert "THIS_GRANT_DOES_NOT_FLIP_NO_VERSIONED=true" in workpaper
    assert "GRANT_MERGE_DOES_NOT_FLIP_NO_VERSIONED=true" in workpaper
    assert "CATALOG_PRODUCE_STILL_FAIL_CLOSES_NO_VERSIONED=true" in workpaper
...
def test_grant_pointers_are_appended_not_rewritten() -> None:
    ...
    assert (
        "s3-a2-incumbent-forecast-artifact-catalog-no-versioned-closeout-authorization.md" in plan
    )
    assert amendment.count(SECTION_208_HEADING) == 1
    assert amendment.count(SECTION_209_HEADING) == 1
    ...
    grant_snapshot = amendment.split("## 209.", 1)[1]
    if "\n## " in grant_snapshot:
    ...
    assert "CATALOG_PRODUCE_STILL_FAIL_CLOSES_NO_VERSIONED=true" in grant_snapshot
    assert "LATER_R1_MAY_RECORD_CATALOG_STILL_FAIL_CLOSES_NO_VERSIONED=true" in grant_snapshot
    assert CONTENT_IDENTITY_SHA256 in grant_snapshot
    assert REVIEW_EVIDENCE_DIGEST_SHA256 in grant_snapshot
    contract_snapshot = amendment.split("## 208.", 1)[1]
    if "## 209." in contract_snapshot:
        contract_snapshot = contract_snapshot.split("## 209.", 1)[0]
...
def test_grant_package_is_docs_only() -> None:
    ...
    assert payload["authorization_merge"]["does_not_flip_no_versioned"] is True
    assert (
        payload["authorization_merge"]["does_not_implement_catalog_no_versioned_closeout"] is True
    )
    assert (
        payload["authorization_merge"]["does_not_add_catalog_no_versioned_closeout_module"] is True
    )
    assert NO_VERSIONED_FLIP_MODULE.is_file()
    assert INDEPENDENT_REVIEW_MODULE.is_file()
    assert LANDING_MODULE.is_file()
...
def test_parent_contract_commit_is_named_for_traceability() -> None:
    ...
    assert payload["parent_contract_tree_sha"] == PARENT_CONTRACT_TREE_SHA
    assert payload["parent_no_versioned_flip_r1_commit"] == PARENT_NO_VERSIONED_FLIP_R1_COMMIT
    assert payload["parent_no_versioned_flip_r1_merge"] == PARENT_NO_VERSIONED_FLIP_R1_MERGE
    assert payload["parent_no_versioned_flip_r1_evidence_json_sha256"] == (
        PARENT_NO_VERSIONED_FLIP_R1_EVIDENCE_JSON_SHA256
    )
    assert payload["parent_no_versioned_flip_r1_pr"] == PARENT_NO_VERSIONED_FLIP_R1_PR
    assert payload["parent_no_versioned_flip_grant_pr"] == PARENT_NO_VERSIONED_FLIP_GRANT_PR
    assert payload["parent_no_versioned_flip_contract_pr"] == PARENT_NO_VERSIONED_FLIP_CONTRACT_PR
    assert payload["parent_independent_review_r1_commit"] == PARENT_INDEPENDENT_REVIEW_R1_COMMIT
    assert payload["parent_independent_review_r1_merge"] == PARENT_INDEPENDENT_REVIEW_R1_MERGE
    assert payload["parent_independent_review_r1_evidence_json_sha256"] == (
        PARENT_INDEPENDENT_REVIEW_R1_EVIDENCE_JSON_SHA256
    )
    assert payload["parent_independent_review_r1_pr"] == PARENT_INDEPENDENT_REVIEW_R1_PR
    assert payload["parent_independent_review_grant_pr"] == PARENT_INDEPENDENT_REVIEW_GRANT_PR
    assert payload["parent_independent_review_contract_pr"] == PARENT_INDEPENDENT_REVIEW_CONTRACT_PR
