"""S3-A2 accepted S2 TRAIN/VAL SOURCE_002 row-level-read live-async-session-connection tests."""
...
AcceptedS2TrainValLiveAsyncSessionConnectionEnvelope = (
    _async_session_connection.AcceptedS2TrainValLiveAsyncSessionConnectionEnvelope
)
...
def test_official_live_async_session_connection_path_fail_closed_or_connected() -> None:
    envelope = obtain_accepted_s2_train_val_async_connection_from_the_already_obtained_live_async_session_connection()  # noqa: E501

    if envelope.connected:
        assert envelope.reason_code is LiveAsyncSessionConnectionReasonCode.CONNECTED
    else:
        assert envelope.reason_code is not LiveAsyncSessionConnectionReasonCode.CONNECTED
    _assert_not_source_002(envelope)


def test_synthetic_sqlite_aiosqlite_connected_is_not_official_live_async_session_connection() -> (
    None
):  # noqa: E501
...
def test_missing_async_session_maker_fail_closes_no_async_session_maker() -> None:
    ...
    assert (
        ...
        is LiveAsyncSessionConnectionReasonCode.FAIL_CLOSED_NO_ASYNC_SESSION_MAKER
    )
    _assert_not_source_002(envelope)
...
def test_session_connection_returns_none_fail_closes_not_obtained_from_session_connection() -> None:
    ...
    assert envelope.connected is False
    assert envelope.reason_code is (
        LiveAsyncSessionConnectionReasonCode.FAIL_CLOSED_ASYNC_CONNECTION_NOT_OBTAINED_FROM_SESSION_CONNECTION
    )
    _assert_not_source_002(envelope)
...
def test_async_session_connection_envelope_has_connected_not_queryable_or_content_bytes() -> None:
    field_names = set(AcceptedS2TrainValLiveAsyncSessionConnectionEnvelope.model_fields)
...
def test_sibling_live_async_session_bind_still_not_source_002() -> None:
...
def test_async_session_connection_module_contains_no_forbidden_patterns() -> None:
    module = Path(
        "backend/app/s3_daily_rowset/"
