"""S3-A2 incumbent forecast artifact no-versioned flip contract tests."""
...
CONTRACT_PATH = Path("docs/v0-3/s3/s3-incumbent-forecast-artifact-no-versioned-flip-contract.md")
WORKPAPER_PATH = Path(
...
PARENT_INDEPENDENT_REVIEW_R1_TEST = Path(
    "backend/tests/s3_daily_rowset/"
    "test_s3_a2_incumbent_forecast_artifact_presence_package_independent_review_r1.py"
)
...
PARENT_INDEPENDENT_REVIEW_R1_TEST_BLOB = "e4a6bc5a6f19c3b447c25004aae98982058f578e"
PARENT_GRANT_PR = 517
...
PARENT_GRANT_TEST_BLOB = "9a5d05ae12b50c5fcc3e5c6e9fe8650fadd79e78"
...
PARENT_INDEPENDENT_REVIEW_CONTRACT_DOC_BLOB = "1e519510b3b7f1d5deb1e3fd9415bcf0f411a280"
PARENT_INDEPENDENT_REVIEW_CONTRACT_WORKPAPER_BLOB = "37a72dc7b6876f709aa5f8c5e1005dc18193b7bc"
PARENT_INDEPENDENT_REVIEW_CONTRACT_EVIDENCE_BLOB = "348e13ea2c5914760d8c8af5a47d41f5c154f372"
PARENT_INDEPENDENT_REVIEW_CONTRACT_TEST_BLOB = "eb0a8807bbbf496335450e988bc19fb4ebf48f66"
...
UNIQUE_FLIP = "S3_A2_INCUMBENT_FORECAST_ARTIFACT_NO_VERSIONED_FLIP_CONTRACT_AUTHORIZED"
THIS_FAMILY_IMPLEMENTATION_AUTHORIZED = (
    "S3_A2_INCUMBENT_FORECAST_ARTIFACT_NO_VERSIONED_FLIP_IMPLEMENTATION_AUTHORIZED"
)
THIS_FAMILY_IMPLEMENTED = "DETERMINISTIC_INCUMBENT_FORECAST_ARTIFACT_NO_VERSIONED_FLIP_IMPLEMENTED"
PARENT_R1_POINTER_HEADING = (
    "#### Incumbent forecast artifact presence-package independent-review R1 pointer"
)
NEW_POINTER_HEADING = "#### Incumbent forecast artifact no-versioned flip contract pointer"
SECTION_205_HEADING = "## 205. Incumbent forecast artifact no-versioned flip contract pointer"
...
def test_frozen_blobs_and_parent_packages_unchanged() -> None:
    ...
    assert _git_blob(PARENT_INDEPENDENT_REVIEW_R1_WORKPAPER) == (
        PARENT_INDEPENDENT_REVIEW_R1_WORKPAPER_BLOB
    )
    assert _git_blob(PARENT_INDEPENDENT_REVIEW_R1_EVIDENCE) == (
        PARENT_INDEPENDENT_REVIEW_R1_EVIDENCE_BLOB
    )
    assert _git_blob(PARENT_INDEPENDENT_REVIEW_R1_TEST) == PARENT_INDEPENDENT_REVIEW_R1_TEST_BLOB
    ...
    assert _git_blob(PARENT_INDEPENDENT_REVIEW_CONTRACT) == (
        PARENT_INDEPENDENT_REVIEW_CONTRACT_DOC_BLOB
    )
    assert _git_blob(PARENT_INDEPENDENT_REVIEW_CONTRACT_WORKPAPER) == (
        PARENT_INDEPENDENT_REVIEW_CONTRACT_WORKPAPER_BLOB
    )
    assert _git_blob(PARENT_INDEPENDENT_REVIEW_CONTRACT_EVIDENCE) == (
        PARENT_INDEPENDENT_REVIEW_CONTRACT_EVIDENCE_BLOB
    )
    assert _git_blob(PARENT_INDEPENDENT_REVIEW_CONTRACT_TEST) == (
        PARENT_INDEPENDENT_REVIEW_CONTRACT_TEST_BLOB
    )
...
def test_production_module_is_filename_only_and_pep420_holds() -> None:
    ...
    assert PRODUCTION_MODULE.name == "s3_a2_incumbent_forecast_artifact_no_versioned_flip.py"
    contract = CONTRACT_PATH.read_text(encoding="utf-8")
    assert "THIS_PR_IS_NOT_R1=true" in contract
    assert "CONTRACT_MERGE_DOES_NOT_IMPLEMENT_NO_VERSIONED_FLIP=true" in contract
    assert f"{THIS_FAMILY_IMPLEMENTED}=false" in contract
    assert not Path("backend/app/s3_daily_rowset/__init__.py").exists()


def test_classify_independent_review_records_digest_and_precondition_4() -> None:
...
def test_frozen_closeouts_still_unauthorized_after_independent_review_classify() -> None:
    IncumbentForecastArtifactPresencePackageIndependentReviewClassifier().classify()
    assert load_reviewed_grain_identity_set() == ()
...
def test_contract_is_authorized_and_not_an_implementation_grant() -> None:
    ...
    assert "CONTRACT_MERGE_DOES_NOT_IMPLEMENT_NO_VERSIONED_FLIP=true" in text
    ...
    assert "LATER_R1_MAY_FLIP_NO_VERSIONED=true" in text
    ...
    assert "NO_VERSIONED_FLIP_PRECONDITION_4_HOLDS=true" in text
    ...
    assert f"PARENT_INDEPENDENT_REVIEW_R1_COMMIT={PARENT_INDEPENDENT_REVIEW_R1_COMMIT}" in text
    assert f"PARENT_INDEPENDENT_REVIEW_R1_MERGE={PARENT_INDEPENDENT_REVIEW_R1_MERGE}" in text
    assert f"PARENT_GRANT_PR={PARENT_GRANT_PR}" in text
    assert f"PARENT_INDEPENDENT_REVIEW_CONTRACT_PR={PARENT_INDEPENDENT_REVIEW_CONTRACT_PR}" in text
    ...
    assert f"INDEPENDENT_REVIEW_PY_BLOB={INDEPENDENT_REVIEW_PY_BLOB}" in text
    lowered = text.lower()
...
def test_evidence_json_sha256_matches_payload_without_self_key() -> None:
    ...
    assert payload["authorization"][
        "s3_a2_incumbent_forecast_artifact_no_versioned_flip_contract_authorized"
    ]
    ...
    assert (
        payload["parent_independent_review_r1"]["parent_independent_review_r1_pr"]
        == PARENT_INDEPENDENT_REVIEW_R1_PR
    )
    assert (
        payload["parent_independent_review_r1"]["parent_independent_review_r1_commit"]
        == PARENT_INDEPENDENT_REVIEW_R1_COMMIT
    )
    assert (
        payload["parent_independent_review_r1"]["parent_independent_review_r1_merge"]
        == PARENT_INDEPENDENT_REVIEW_R1_MERGE
    )
    assert payload["parent_grant"]["parent_grant_pr"] == PARENT_GRANT_PR
    assert (
        payload["parent_independent_review_contract"]["parent_independent_review_contract_pr"]
        == PARENT_INDEPENDENT_REVIEW_CONTRACT_PR
    )
    ...
    assert payload["no_versioned_flip_preconditions"]["no_versioned_flip_precondition_1_holds"]
    assert payload["no_versioned_flip_preconditions"]["no_versioned_flip_precondition_2_holds"]
    assert payload["no_versioned_flip_preconditions"]["no_versioned_flip_precondition_3_holds"]
    assert payload["no_versioned_flip_preconditions"]["no_versioned_flip_precondition_4_holds"]
    assert payload["reviewed_artifact"]["later_r1_may_flip_no_versioned"] is True
...
def test_development_plan_live_compact_flips_only_this_contract() -> None:
    ...
    assert (
        "s3-a2-incumbent-forecast-artifact-presence-package-independent-review-r1.md" in r1_pointer
    )
    assert (
        "s3-incumbent-forecast-artifact-no-versioned-flip-contract.md"
    ...
    assert "LATER_R1_MAY_FLIP_NO_VERSIONED=true" in CONTRACT_PATH.read_text(encoding="utf-8")
    assert "LATER_R1_MUST_NOT_FLIP_NO_VERSIONED=true" not in CONTRACT_PATH.read_text(
        encoding="utf-8"
    )


def test_amendment_records_pointer_and_isolates_section_204() -> None:
    text = AMENDMENT.read_text(encoding="utf-8")
    assert text.count("## 204.") == 1
    assert text.count(SECTION_205_HEADING) == 1
    assert f"{UNIQUE_FLIP}=true" in text
    section_204 = text.split("## 204.", 1)[1]
    if "## 205." in section_204:
        section_204 = section_204.split("## 205.", 1)[0]
    assert UNIQUE_FLIP not in section_204
    assert (
        "DETERMINISTIC_INCUMBENT_FORECAST_ARTIFACT_PRESENCE_PACKAGE_INDEPENDENT_REVIEW_IMPLEMENTED=true"
        in section_204
    )
    assert "NO_VERSIONED_INCUMBENT_FORECAST_ARTIFACT_IN_REPOSITORY=true" in section_204
    assert CONTENT_IDENTITY_SHA256 in section_204
    section_205 = text.split("## 205.", 1)[1]
    assert f"{UNIQUE_FLIP}=true" in section_205
    assert f"{THIS_FAMILY_IMPLEMENTATION_AUTHORIZED}=false" in section_205
    assert f"{THIS_FAMILY_IMPLEMENTED}=false" in section_205
    assert "NO_VERSIONED_FLIP_PRECONDITION_4_HOLDS=true" in section_205
    assert "NO_VERSIONED_INCUMBENT_FORECAST_ARTIFACT_IN_REPOSITORY=true" in section_205
    assert "S3_A2_COMPLETENESS_PASS_AUTHORIZED=false" in section_205
    assert CONTENT_IDENTITY_SHA256 in section_205
    assert REVIEW_EVIDENCE_DIGEST_SHA256 in section_205
    assert REVIEWED_SET_IDENTITY_SHA256 in section_205
    assert f"BASE_MAIN_SHA={BASE_MAIN_SHA}" in section_205
    assert f"EVIDENCE_JSON_SHA256={THIS_CONTRACT_EVIDENCE_JSON_SHA256}" in section_205
    lowered = section_205.lower()
    for token in FORBIDDEN_PROSE_TOKENS:
        assert token.lower() not in lowered
