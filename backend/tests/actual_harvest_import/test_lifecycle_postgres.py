...
@pytest.mark.asyncio
async def test_postgres_i5_committed_finalized_predecessor_finalized_at_persists_in_basis() -> None:
    """Migration 0022 contract: a committed FINALIZED predecessor's
    finalized_at must survive into the lineage basis member row so the
    I7 AS_OF_FINALIZED_AFTER_CUTOFF and I5 FINALIZED_AT_REQUIRED checks
    can read it on subsequent replays (re-validation or I7 snapshot
    creation). Round-trip assertion: seed a committed predecessor
    whose record_status=FINALIZED with a non-null finalized_at; re-
    validate against a successor; assert the persisted basis member
    ...
    first_revision = f"revision-first-{suffix}"
    second_revision = f"revision-second-{suffix}"
    predecessor_finalized_at = datetime(2024, 5, 1, 12, 0, tzinfo=UTC)
    first_id, first_external = await _seed_i5_batch_with_record(
        suffix=f"first-{suffix}",
        mapping_policy=mapping_policy,
        logical_id=logical_id,
        ...
        async with AsyncSessionMaker() as session:
            ...
            persisted_member = await session.scalar(
                sa.select(ActualHarvestValidationLineageBasisMemberModel).where(
                    ActualHarvestValidationLineageBasisMemberModel.basis_id == basis.id,
                    ActualHarvestValidationLineageBasisMemberModel.external_revision_id
                    == first_revision,
                )
            )
            assert persisted_member is not None
            # Migration 0022 contract: finalized_at round-trips through
            # committed lineage basis evidence exactly.
            assert persisted_member.finalized_at is not None
            assert _utcify(persisted_member.finalized_at) == predecessor_finalized_at
            assert persisted_member.record_status == (ActualHarvestRecordStatus.FINALIZED.value)
    finally:
        await _cleanup_batch(first_external)
        await _cleanup_batch(second_external)
...
@pytest.mark.asyncio
async def test_postgres_i5_committed_finalized_predecessor_null_finalized_at_required() -> None:
    """Migration 0022 contract negative path: a committed predecessor
    whose record_status=FINALIZED but whose finalized_at IS NULL must
    be rejected by the I5 hardening check (FINALIZED_AT_REQUIRED)
    even when its successor is also FINALIZED. The basis member row
    must NOT be persisted for the offending predecessor (transaction
    rollback), and the error code must surface through the public
    ...
    try:
        ...
        async with AsyncSessionMaker() as session:
            error_codes = (
                (
                    await session.execute(
                        sa.select(ActualHarvestValidationErrorModel.error_code)
                        .join(
                            ActualHarvestValidationRunModel,
                            ActualHarvestValidationRunModel.id
                            == ActualHarvestValidationErrorModel.validation_run_id,
            ...
            assert basis is None, (
                "lineage basis must not persist when FINALIZED_AT_REQUIRED rejected"
            )
    finally:
        await _cleanup_batch(first_external)
        await _cleanup_batch(second_external)
...
def _utcify(value: datetime) -> datetime:
    """Normalize a SQLAlchemy-persisted datetime to a UTC-aware datetime
    so the round-trip assertion is not sensitive to the +00:00 vs
    tzinfo=None mismatch the ORM can produce on some PostgreSQL
    driver/options combos.
    """
