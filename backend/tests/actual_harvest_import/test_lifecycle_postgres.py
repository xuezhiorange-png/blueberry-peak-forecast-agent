...
@pytest.mark.asyncio
async def test_postgres_i7_committed_finalized_at_persists_to_basis_member() -> None:
    """Round-trip of migration 0022 (lineage basis member
    ``finalized_at``).

    A committed FINALIZED record with ``finalized_at = NULL`` MUST
    persist that null onto the ``actual_harvest_validation_lineage_
    basis_member`` row. A subsequent validation that reloads the
    committed source universe MUST surface the persisted
    ``finalized_at = NULL`` and raise ``FINALIZED_AT_REQUIRED``.

    This is the I7 acceptance test for the 0022 column addition; it
    is NOT deferred to I8.
    """
    _require_postgres()
    suffix = uuid4().hex
    mapping_policy = await _seed_i5_registry(suffix)
    logical_id = f"logical-i7-fin-null-{suffix}"
    revision_id = f"revision-i7-fin-null-{suffix}"
    first_id, first_external = await _seed_i5_batch_with_record(
        suffix=f"first-{suffix}",
        mapping_policy=mapping_policy,
        logical_id=logical_id,
        ...
        record_updates={
            ...
            # Intentionally null; the column was just added and we
            # want to assert the validator reads it back the same way.
            "finalized_at": None,
        },
        seal=False,
    )
    second_id, second_external = await _seed_i5_batch_with_record(
        ...
        mapping_policy=mapping_policy,
        logical_id=logical_id,
        revision_id=f"revision-i7-fin-null-child-{suffix}",
        revision_number=2,
        predecessor=revision_id,
    )
    try:
        # Seal both batches and validate the first one (so its
        # lineage basis member is persisted with finalized_at = NULL).
        async with AsyncSessionMaker() as session:
            async with session.begin():
        ...
        first_validation = await _validate_once(first_id)
        assert first_validation.validation_status == "VALIDATED"

        # Now verify the lineage basis member row carries
        # finalized_at = NULL.
        async with AsyncSessionMaker() as session:
            member = await session.scalar(
                sa.select(ActualHarvestValidationLineageBasisMemberModel).where(
                    ActualHarvestValidationLineageBasisMemberModel.external_revision_id
                    == revision_id
                )
            )
            assert member is not None
            assert member.record_status == ActualHarvestRecordStatus.FINALIZED.value
            assert member.finalized_at is None

        # Commit the first batch so it becomes a prior committed
        # source for the second validation.
        async with AsyncSessionMaker() as session:
            async with session.begin():
                first_batch_row = await session.scalar(
                    sa.select(ActualHarvestImportBatchModel).where(
                        ActualHarvestImportBatchModel.import_id == first_id
                    )
                )
                assert first_batch_row is not None
                first_batch_row.status = ActualHarvestImportBatchStatus.COMMITTED.value
                first_batch_row.committed_at_or_null = datetime.now(UTC)

        # Validating the second batch reloads the lineage basis
        # members from the prior committed batch. The persisted
        # ``finalized_at = NULL`` MUST trigger FINALIZED_AT_REQUIRED
        # on the prior FINALIZED record in the lineage basis.
        second_validation = await _validate_once(second_id)
        assert second_validation.validation_status == "VALIDATION_FAILED"
        codes = await _validation_error_codes(second_id)
        assert "FINALIZED_AT_REQUIRED" in codes, (
            f"Expected FINALIZED_AT_REQUIRED on the second batch; got error codes: {sorted(codes)}"
        )
    finally:
        await _cleanup_batch(first_external)
        await _cleanup_batch(second_external)
