...
async def test_postgres_authority_classifier_bare_default_success(
    transactional_pg_session: AsyncSession,
) -> None:
    from unittest.mock import patch
    ...
    with (
        ...
        result = authority_mod.DefaultCatalogLiveBindabilityAndRegistryAvailabilityClassifier(
            dataset_identity=DATASET_IDENTITY,
        ).classify()

    assert result.reason_code is authority_mod.AuthorityReasonCode.LIVE_BINDABLE_CATALOG
    assert result.authority_classification is authority_mod.AuthorityClassification.LIVE_BINDABLE
    assert result.binding_classification.value == "NOT_BINDABLE"
    assert result.binding_reason_code.value == "NOT_BINDABLE"
    assert result.in_memory_structural_acceptance is True
    assert result.catalog_identity_sha256 == (
        "00f6bc532dfd97f2d625fc1347bf2a7663299fda206bd472df4c2c32c54ab5af"
    )
    assert result.catalog_entry_count == 2427
