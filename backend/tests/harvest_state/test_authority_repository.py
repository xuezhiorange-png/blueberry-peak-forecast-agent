...
from backend.app.harvest_state.authority_repository import (
    ...
    _FAMILY_REVISION_COLUMN_NAME,
    _FAMILY_VERSION_COLUMN_NAME,
    _advisory_lock_key,
    _extract_scope,
)
...
# ═══════════════════════════════════════════════════════════════════════
# 8. P0-7 exact-load version/revision column mapping
# ═══════════════════════════════════════════════════════════════════════


class TestExactLoadColumnMapping:
    """Every AuthorityFamily has a version and revision column mapping."""

    def test_all_families_have_version_column(self):
        for family in AuthorityFamily:
            assert family in _FAMILY_VERSION_COLUMN_NAME, (
                f"Missing version column for {family}"
            )

    def test_all_families_have_revision_column(self):
        for family in AuthorityFamily:
            assert family in _FAMILY_REVISION_COLUMN_NAME, (
                f"Missing revision column for {family}"
            )

    def test_daily_capacity_uses_daily_capacity_revision(self):
        assert _FAMILY_REVISION_COLUMN_NAME[AuthorityFamily.DAILY_CAPACITY] == (
            "daily_capacity_revision"
        )

    def test_mature_loss_uses_revision(self):
        assert _FAMILY_REVISION_COLUMN_NAME[
            AuthorityFamily.MATURE_INVENTORY_LOSS_AUTHORITY
        ] == "revision"

    def test_pool_version_column(self):
        assert _FAMILY_VERSION_COLUMN_NAME[
