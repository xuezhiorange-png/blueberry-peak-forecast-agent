...
def test_duplicate_grain_emits_error_findings(
    cleaning_build_request: CleaningBuildRequest,
    synthetic_batch,
    synthetic_artifact,
    ...
    assert all(finding.quality_rule_id == DUPLICATE_GRAIN_RULE_ID for finding in duplicate_findings)
    assert all(finding.severity == QualityFindingSeverity.ERROR for finding in duplicate_findings)
