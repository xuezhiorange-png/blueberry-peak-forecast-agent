...
async def _resolve_business_grain(
...
async def _visible_forecast_run_count_for_grain(
    session: AsyncSession,
    *,
    season_id: int,
    farm_id: int,
...
async def _visible_forecast_run_for_grain(
    session: AsyncSession,
    *,
    season_id: int,
    farm_id: int,
    ...
    count = await _visible_forecast_run_count_for_grain(
        session,
        season_id=season_id,
        farm_id=farm_id,
...
async def _append_daily_cells(
    ...
    *,
    ...
    if core_run is None:
        return
    task10_prediction_run_id = await lookup_task10_prediction_run_id(
        session,
...
async def build_pit_visible_incumbent_daily_curve_index(
    session: AsyncSession,
    *,
    forecast_cutoff_at: datetime,
    ...
    for season, farm, subfarm, variety in sorted(grains):
        ...
        run_count = await _visible_forecast_run_count_for_grain(
            session,
            season_id=season_id,
            farm_id=farm_id,
            subfarm_id=subfarm_id,
        ...
        if run_count != 1:
            continue
        forecast_run = await _visible_forecast_run_for_grain(
            session,
            season_id=season_id,
            farm_id=farm_id,
            subfarm_id=subfarm_id,
        ...
        if forecast_run is None:
            continue
        daily_rows = await session.scalars(
