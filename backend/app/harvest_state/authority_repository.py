...
def _advisory_lock_key(
    family: str, business_key: str, version: str, revision: int
) -> int:
...
def _stable_key_from_orm_capacity_pool(row: Task9CapacityPoolDefinition) -> str:
    return (
        f"capacity-pool:{row.season_id}:{row.destination_factory_id}:"
        f"{row.capacity_pool_code}"
    )
...
def _stable_key_from_orm_run_package(row: Task9RunParameterPackage) -> str:
    return (
        f"run-package:{row.season_id}:{row.destination_factory_id}:"
        f"{row.farm_scope_key}"
    )
...
async def create_or_load_capacity_pool_definition(
    ...
    *,
    ...
    stmt = select(Task9CapacityPoolDefinition).where(
        ...
        == definition.destination_factory_id,
        Task9CapacityPoolDefinition.capacity_pool_code
        == definition.capacity_pool_code,
        Task9CapacityPoolDefinition.capacity_pool_version
        == definition.capacity_pool_version,
    ...
    if existing is not None:
        ...
        member_stmt = (
            ...
            .where(
                Task9CapacityPoolMember.capacity_pool_definition_id == existing.id
            )
            .order_by(
    ...
    for member in sorted(
        ...
        member_hash = make_authority_row_hash(
            member, parent_definition=definition
        )
        child_row = Task9CapacityPoolMember(
            ...
            effective_to=definition.effective_to,
            status=AuthorityStatus.DRAFT,
            consumable_from_key=definition.available_at_local_date,
            consumable_to_key=definition.available_at_local_date,  # placeholder
    ...
    child_stmt = (
        select(Task9CapacityPoolMember)
        .where(
            Task9CapacityPoolMember.capacity_pool_definition_id == parent_row.id
        )
        .order_by(
            Task9CapacityPoolMember.farm_id,
            Task9CapacityPoolMember.subfarm_id,
...
async def load_capacity_pool_definition_by_id(
    ...
    *,
    ...
    member_stmt = (
        select(Task9CapacityPoolMember)
        .where(
            Task9CapacityPoolMember.capacity_pool_definition_id == row.id
        )
        .order_by(
            Task9CapacityPoolMember.farm_id,
            Task9CapacityPoolMember.subfarm_id,
...
async def create_or_load_daily_capacity(
    ...
    *,
    ...
    pool_stmt = select(Task9CapacityPoolDefinition).where(
        ...
        Task9CapacityPoolDefinition.destination_factory_id
        == daily_input.destination_factory_id,
        Task9CapacityPoolDefinition.capacity_pool_code
        == daily_input.capacity_pool_code,
        Task9CapacityPoolDefinition.capacity_pool_version
        == daily_input.capacity_pool_version,
    ...
    stmt = select(Task9DailyCapacityAuthority).where(
        ...
        Task9DailyCapacityAuthority.daily_capacity_revision
        == daily_input.daily_capacity_revision,
    )
    result = await session.execute(stmt)
    existing = result.scalar_one_or_none()
...
async def create_or_load_holiday_calendar(
    ...
    *,
    ...
    stmt = select(Task9HolidayCalendarVersion).where(
        ...
        == header.lifecycle_timezone_name,
    ...
    if existing is not None:
        ...
        date_stmt = (
            ...
            .where(
                Task9HolidayCalendarDate.holiday_calendar_version_id == existing.id
            )
            .order_by(
    ...
    child_stmt = (
        select(Task9HolidayCalendarDate)
        .where(
            Task9HolidayCalendarDate.holiday_calendar_version_id == parent_row.id
        )
        .order_by(
            Task9HolidayCalendarDate.holiday_date,
            Task9HolidayCalendarDate.holiday_code,
...
async def load_holiday_calendar_by_id(
    ...
    *,
    ...
    date_stmt = (
        select(Task9HolidayCalendarDate)
        .where(
            Task9HolidayCalendarDate.holiday_calendar_version_id == row.id
        )
        .order_by(
            Task9HolidayCalendarDate.holiday_date,
            Task9HolidayCalendarDate.holiday_code,
...
async def create_or_load_run_parameter_package(
    ...
    *,
    ...
    holiday_stmt = select(Task9HolidayCalendarVersion).where(
        ...
        Task9HolidayCalendarVersion.calendar_version
        == holiday_calendar.calendar_version,
    ...
    date_stmt = (
        select(Task9HolidayCalendarDate)
        .where(
            Task9HolidayCalendarDate.holiday_calendar_version_id == holiday_row.id
        )
        .order_by(
            Task9HolidayCalendarDate.holiday_date,
            Task9HolidayCalendarDate.holiday_code,
    ...
    stmt = select(Task9RunParameterPackage).where(
        ...
        Task9RunParameterPackage.destination_factory_id
        == package_input.destination_factory_id,
    ...
    if existing is not None:
        ...
        existing_holiday_stmt = select(Task9HolidayCalendarVersion).where(
            Task9HolidayCalendarVersion.id
            == existing.holiday_calendar_version_id
        )
        ...
        existing_weather_stmt = select(Task9WeatherRuleConfigVersion).where(
            Task9WeatherRuleConfigVersion.id
            == existing.weather_rule_config_version_id
        )
        ...
        existing_date_stmt = (
            select(Task9HolidayCalendarDate)
            .where(
                Task9HolidayCalendarDate.holiday_calendar_version_id
                == existing_holiday.id
            )
            .order_by(
                Task9HolidayCalendarDate.holiday_date,
...
async def create_or_load_initial_inventory(
    ...
    *,
    ...
    stmt = select(Task9InitialInventorySnapshot).where(
        ...
        Task9InitialInventorySnapshot.destination_factory_id
        == snapshot.destination_factory_id,
        Task9InitialInventorySnapshot.opening_state_date
        == snapshot.opening_state_date,
        Task9InitialInventorySnapshot.snapshot_version
        == snapshot.snapshot_version,
    ...
    if existing is not None:
        ...
        cohort_stmt = (
            ...
            .where(
                Task9InitialInventoryCohort.initial_inventory_snapshot_id
                == existing.id
            )
...
async def load_initial_inventory_by_id(
    ...
    *,
    ...
    cohort_stmt = (
        select(Task9InitialInventoryCohort)
        .where(
            Task9InitialInventoryCohort.initial_inventory_snapshot_id == row.id
        )
        .order_by(Task9InitialInventoryCohort.stable_cohort_key)
    )
    cohort_result = await session.execute(cohort_stmt)
...
async def create_or_load_mature_loss(
    ...
    *,
    ...
    stmt = select(Task9MatureInventoryLossAuthority).where(
        ...
        Task9MatureInventoryLossAuthority.capacity_pool_code
        == loss_input.capacity_pool_code,
        Task9MatureInventoryLossAuthority.forecast_quantile
        == loss_input.forecast_quantile.value,
...
async def activate_authority(
    ...
    *,
    ...
    stable_key, version, revision = await _resolve_stable_key_and_version(
        family, row
    )

    await _validate_transition(
        row.status, AuthorityStatus.ACTIVE, family, stable_key
    )
...
async def cancel_authority(
    ...
    *,
    ...
    await _validate_transition(
        row.status, AuthorityStatus.CANCELLED, family, stable_key
    )
...
async def retire_authority(
    ...
    *,
    ...
    await _validate_transition(
        row.status, AuthorityStatus.RETIRED, family, stable_key
    )
...
async def supersede_authority(
    ...
    *,
    ...
    """
    # (1) Lock old
    old_row = await _load_for_update(
        session, family=family, authority_id=old_id
    )
    ...
    await _validate_transition(
        old_row.status, AuthorityStatus.SUPERSEDED, family, old_stable_key
    )
    ...
    if family == AuthorityFamily.CAPACITY_POOL_DEFINITION:
        ...
        new_result = await create_or_load_capacity_pool_definition(
            session, definition_input=bundle
        )
    elif family == AuthorityFamily.DAILY_CAPACITY:
        new_result = await create_or_load_daily_capacity(
            session, daily_input=new_input
        )
        ...
        new_result = await create_or_load_holiday_calendar(
            session, calendar_input=bundle
        )
    elif family == AuthorityFamily.WEATHER_RULE_CONFIG_VERSION:
        new_result = await create_or_load_weather_rule(
            session, weather_input=new_input
        )
        ...
        new_result = await create_or_load_initial_inventory(
            session, inventory_input=bundle
        )
    elif family == AuthorityFamily.MATURE_INVENTORY_LOSS_AUTHORITY:
        new_result = await create_or_load_mature_loss(
            session, loss_input=new_input
        )
    else:
...
async def replace_run_package_with_dependencies(
    ...
    *,
    ...
    _new_holiday = await create_or_load_holiday_calendar(
        session, calendar_input=new_holiday_input
    )
    _new_weather = await create_or_load_weather_rule(
