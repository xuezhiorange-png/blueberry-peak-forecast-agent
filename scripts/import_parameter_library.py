...
from backend.app.planning.importers import import_parameter_library_csv
from backend.app.planning.schemas import ImportExecutionResult


def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Import Task 5 parameter library CSV.")
...
def _write_report(report_dir: Path, version: str, result: ImportExecutionResult) -> None:
    ...
    output_path = report_dir / f"parameter-library-import-{version}.json"
...
async def _run() -> int:
    ...
    async with AsyncSessionMaker() as session:
        result = await import_parameter_library_csv(
            session,
