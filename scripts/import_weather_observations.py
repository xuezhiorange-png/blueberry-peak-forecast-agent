...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Import Task 7 weather daily observations CSV.")
...
def _write_report(output_dir: Path, file_name: str, payload: dict[str, object]) -> None:
    ...
    path = output_dir / f"weather-observations-{Path(file_name).stem}.json"
    path.write_text(
...
async def _run() -> int:
    ...
    from backend.app.weather.service import import_weather_observations

    args = _parser().parse_args()
    logging.basicConfig(level=getattr(logging, str(args.log_level).upper(), logging.INFO))
    async with AsyncSessionMaker() as session:
        result = await import_weather_observations(
            session,
