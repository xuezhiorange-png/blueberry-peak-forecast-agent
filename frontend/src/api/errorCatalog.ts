export type TrialErrorCode =
  ...
  | "TRIAL_AUTHORITY_NOT_FOUND"
  | "MARKETABLE_RETENTION_POLICY_MISSING"
  | "MARKETABLE_RETENTION_POLICY_CONFLICT"
  | "TRIAL_FORECAST_BLOCKED"
  | "TRIAL_IMPORT_PARSE_FAILED"
  | "TRIAL_IMPORT_VALIDATION_FAILED"
  | "TRIAL_QUALITY_NOT_COMPUTABLE"
  | "TRIAL_PARTIAL_RESULT_REJECTED";
...
const messages: Record<TrialErrorCode, string> = {
  ...
  TRIAL_AUTHORITY_NOT_FOUND: "输入范围当前不可用。",
  MARKETABLE_RETENTION_POLICY_MISSING: "预测策略暂不可用，请稍后重试。",
  MARKETABLE_RETENTION_POLICY_CONFLICT: "预测策略证据存在冲突，无法生成结果。",
  TRIAL_FORECAST_BLOCKED: "预测被服务端证据阻断，未生成可展示结果。",
  TRIAL_IMPORT_PARSE_FAILED: "文件解析失败，请检查文件格式。",
  TRIAL_IMPORT_VALIDATION_FAILED: "文件校验失败，请查看服务端返回的错误行。",
  TRIAL_QUALITY_NOT_COMPUTABLE: "质量指标当前不可计算。",
  TRIAL_PARTIAL_RESULT_REJECTED: "服务端返回了不完整结果，已停止展示。",
};
