...
function isValidRfc3339(value: string): boolean {
  ...
  return offset !== null && Number(offset[1]) <= 23 && Number(offset[2]) <= 59;
}
...
export const dateSchema = z.string().regex(DATE).refine(isValidCalendarDate);
export const timestampSchema = z.string().regex(RFC3339).refine(isValidRfc3339);
